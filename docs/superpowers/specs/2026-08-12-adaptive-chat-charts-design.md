...
| `packages/flutter_adaptive_charts_fs/test/widgets_barrel_test.dart` (new) | The widgets barrel resolves every class it promises: each registry builder is called and its result checked against the exported widget type, and `BarChartType` is confirmed to have its four values. Dart has no runtime way to assert a symbol is _not_ exported, so this test does not check that the internal chrome (`ChartChrome`, `GaugePainter`, …) stays unreachable — that rests on the barrel's `show` clause alone, not on a test. |
