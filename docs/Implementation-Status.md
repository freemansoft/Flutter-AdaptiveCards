...
| ActionSet     | [spec](https://adaptivecards.io/explorer/ActionSet.html)     | ⚠️ Partial     | ⚠️ Limited | -                                                                                                                        | Actions beyond `maxActions` are silently dropped (`.take()`); action `mode: secondary` / overflow "•••" menu not implemented — see [addendum A3](./superpowers/specs/2026-06-17-spec-compliance-audit-addendum.md) |
...
| `verticalContentAlignment`     | spec (v1.1)                                                 | ⚠️ Partial     | Not applied at card **root**; **is** implemented at Container/Column/Table (`cards/containers/*.dart`) — see [addendum §B](./superpowers/specs/2026-06-17-spec-compliance-audit-addendum.md)                                                  |
...
> [!WARNING]
> **Validation gap:** the live submit/execute gate is `validateInputs()` (`action/default_actions.dart`), which enforces only `isRequired` for all inputs plus `regex` for `Input.Text`. **`Input.Number` / `Input.Date` / `Input.Time` `min`/`max` are not enforced** — out-of-range values submit silently. The per-input `checkRequired()` methods below are **not wired** to this gate (only a test calls them). See [audit addendum A1 + A7](./superpowers/specs/2026-06-17-spec-compliance-audit-addendum.md).
...
| Adaptive Expressions | [spec](https://learn.microsoft.com/en-us/azure/bot-service/adaptive-expressions/adaptive-expressions-prebuilt-functions) | ⚠️ Partial     | ✅ Yes | -                                                            | Operators, string, math, logic, and `formatDateTime` implemented; other date functions (`utcNow`, `addDays`, `formatEpoch`) and collection functions (`select`, `where`, `join`) missing |
...
| **Input min/max validation**    | `min`/`max` not enforced on submit for Number/Date/Time; invalid values submit silently              | **High — data integrity** ([addendum A1](./superpowers/specs/2026-06-17-spec-compliance-audit-addendum.md)) |
| **CodeBlock key**               | Reads `code` instead of spec `codeSnippet` → empty render                                            | Medium — confirmed bug ([addendum A2](./superpowers/specs/2026-06-17-spec-compliance-audit-addendum.md)) |
| **Action `mode` / overflow**    | `mode: secondary` and overflow menu unimplemented; excess actions dropped                            | Medium ([addendum A3](./superpowers/specs/2026-06-17-spec-compliance-audit-addendum.md)) |

> [!NOTE]
> Finer-grained per-property gaps inside elements/inputs/actions marked ✅ above are catalogued in the [spec-compliance audit addendum (2026-06-17)](./superpowers/specs/2026-06-17-spec-compliance-audit-addendum.md).
...
| ----------------- | ---------------- | -------------- | ---------- | ----------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| Badge             | `Badge`          | ⚠️ Partial     | ⚠️ Limited | -                                                                 | Hub extension; HostConfig `BadgeStylesConfig`. `icon`, `shape`, color `style` not read — see [audit addendum A4](./superpowers/specs/2026-06-17-spec-compliance-audit-addendum.md) |
| Carousel          | `Carousel`       | ⚠️ Partial     | ⚠️ Limited | -                                                                 | Hub extension; child pages use `CarouselPage`. `timer`/auto-advance, `orientation`, `loop` not read — see [addendum A6](./superpowers/specs/2026-06-17-spec-compliance-audit-addendum.md) |
...
| CodeBlock         | `CodeBlock`      | ⚠️ Partial     | ⚠️ Limited | -                                                                 | Hub / Teams extension. **Bug:** reads `code` instead of spec `codeSnippet` → empty render — see [addendum A2](./superpowers/specs/2026-06-17-spec-compliance-audit-addendum.md) |
| CompoundButton    | `CompoundButton` | ⚠️ Partial     | ⚠️ Limited | -                                                                 | Hub / Teams extension. `icon`, `badge` not read — see [addendum A5](./superpowers/specs/2026-06-17-spec-compliance-audit-addendum.md) |
