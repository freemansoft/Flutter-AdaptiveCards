...
| Image         | [spec](https://adaptivecards.io/explorer/Image.html)         | ✅ Complete    | ✅ Yes     | [Encoded-Image-Support.md](./Encoded-Image-Support.md)                                                                   | Supports base64; `selectAction` and `backgroundColor` supported                                                                             |
...
| CaptionSource | [spec](https://adaptivecards.io/explorer/CaptionSource.html) | ⚠️ Partial     | ✅ Yes     | -                                                                                                                        | Parsed into typed `CaptionSource` model (`captionSourcesFromJsonList`); VTT track rendering on the video surface not yet wired              |
...
| ActionSet     | [spec](https://adaptivecards.io/explorer/ActionSet.html)     | ✅ Complete    | ✅ Yes     | -                                                                                                                        | Action `mode: secondary` and actions beyond `maxActions` collapse into a reveal "•••" overflow (also wired for card-level actions)          |
...
| **Templating `select`/`where`** | Require lazy lambda evaluation (parser change); other collection/date functions implemented          | Low — advanced templating                                        |
...
| CompoundButton    | `CompoundButton` | ✅ Complete    | ✅ Yes     | -                                                                 | Hub / Teams extension. Supports `title`, `description`, `iconUrl`, `badge` (trailing pill); golden-tested                                                                                    |
