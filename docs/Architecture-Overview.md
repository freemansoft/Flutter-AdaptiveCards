...
flowchart TB
  ...
  subgraph widthScope["LayoutBuilder nested ProviderScope"]
    WB["cardWidthBucketProvider - responsive WidthBucket"]
  end
...
| Action dispatch `flowchart` + `sequenceDiagram`                                       | **Yes** (new)            | [actions-architecture.md](./actions-architecture.md#action-dispatch-overview)                                                                                       | Registry → `Default*Action.tap` → handler; host README carries a summary + link, not canon                                       |
| Backend invoke round-trip `sequenceDiagram`                                           | **Yes** (new)            | [backend-host-integration.md](./backend-host-integration.md#architecture)                                                                                           | Temporal view; complements the pipeline `flowchart LR`                                                                           |
| Sign-in round-trip `sequenceDiagram`                                                  | **Yes** (new)            | [backend-host-integration.md](./backend-host-integration.md#sign-in-authentication)                                                                                 | Two-phase `urlOpener` → `completeSignin` (root `authentication`)                                                                 |
...
| Backend host design spec diagram                                                      | **Duplicate (archived)** | Use [backend-host-integration.md](./backend-host-integration.md)                                                                                                    | [`archive/specs/2026-06-07-backend-host-integration-design.md`](./archive/specs/2026-06-07-backend-host-integration-design.md)   |
| HostConfig style pipeline spec (2 diagrams)                                           | **Duplicate (archived)** | Use [adaptive-style.md](./adaptive-style.md)                                                                                                                        | [`archive/specs/2026-06-06-hostconfig-style-pipeline-design.md`](./archive/specs/2026-06-06-hostconfig-style-pipeline-design.md) |
