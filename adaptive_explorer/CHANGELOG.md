# Changelog

## [0.18.0]

- no changes yet

## [0.17.0]

- chore: require Dart SDK ^3.13.0 and Flutter 3.47.4 (FVM pin, CI pins, `very_good_analysis` ^11.0.0). Constructors use the Dart 3.13 abbreviated in-body syntax (`new(...)`, `factory(...)`) and the new formatter output, as required by the lints `very_good_analysis` 11 enables.
- refactor: classes use Dart 3.13 primary constructors with declaring parameters; field docs sit on the parameters in the class header and constructor docs on the in-body `this` declaration. `use_declaring_parameters` is enforced by `analysis_options.yaml`. No behavior change; public constructor signatures are unchanged.
- chore: platform folders regenerated in place from the Flutter 3.47.4 `flutter create` template: macOS 12.0 deployment target, Swift Package Manager workspace references, the Windows `utils.cpp` bounds fix (CWE-126), and the macOS sandbox capability keys. Re-applied the macOS `network.client` and `files.user-selected.read-write` entitlements.

## [0.16.0]

- no changes yet

## [0.15.0]

- no changes yet

## [0.14.0]

- no changes yet

## [0.13.0]

- no changes yet

## [0.12.0]

- no changes yet

## [0.11.0]

- no changes yet

## [0.10.0]

- **README** no longer claims chart element support; documents cards + template only and points to widgetbook for chart samples.
- Removed unused **`cupertino_icons`** and **`path`** dependencies from `pubspec.yaml`.

## [0.9.0]

- Version alignment with library packages 0.9.0.

## [0.8.0]

- Updated to Dart SDK 3.12 and Flutter 3.44
- Migrated to Swift Package Manager from CocoaPods

## [0.7.0]

- Change landscape / portrait breakpoint handling

## [0.6.0]

- Bumped versions to 0.6.0 for next development cycle
- Updated to Dart SDK 3.11 and Flutter 3.41

## [0.5.0] - 2026-04-19

- version numbers were sync'd to 0.5.0
- moved the TabBar from the AppBar to directly above the JSON editor panes
- changed default JSON viewer from tree to text mode

## [0.4.0] - 2026-04-14

- version numbers were sync'd to the flutter_adaptive_charts_fs 0.4.0 version
