...
void main() {
  group('probeOnce timeout', () {
    ...
    setUp(() async {
      ...
      //
      // `/api/generate` answers promptly, because the real server does: a
      // timed-out call evicts the runner so the abandoned generation stops,
      // and a fake that stalled that too would model an Ollama that cannot
      // unload — and would charge every timeout test the eviction bound.
    ...
    test(
      'evicts the runner, so the next call is not queued behind it',
      () async {
        ...
        // 31 where two were real. Asserting the unload is issued is what keeps
