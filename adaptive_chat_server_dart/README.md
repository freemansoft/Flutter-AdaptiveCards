...
| `assets/card_tool_prompt_matched.txt`     | Bundled **tool-channel** palette, used by the probes (`tool_call_probe.dart`, `shape_ab.dart --channel tool`) rather than by the server, which has no tool-channel mode. `card_system_prompt.txt` with only the raw-JSON-emission mechanics rewritten and a byte-identical element catalogue, so a prose/tool delta is a property of the channel rather than of two differently written prompts. |
...
prompts, and is re-read per request — so it can be re-tuned without a
rebuild, and `--seed-card-file` points at a candidate for A/B runs the way
`--system-prompt-file` does for prompts. The file is a JSON array of
`{"role", "content"}` turns whose roles must alternate from `user`:

```json
[
  {
    "role": "user",
    "content": "what timezone should I use for the nightly build?"
  },
  { "role": "assistant", "content": "[{\"type\":\"TextBlock\", ...}]" }
]
```
