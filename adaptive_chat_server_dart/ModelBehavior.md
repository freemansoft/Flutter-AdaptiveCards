...
### Two readings hold on both models; two are a confirmed miss on the large model; retry-after-abort is unstable on it
...
| same system prompt, different question                 | 2144 / 2136                       | 87 ms           | 3177 / 4, 3177 / 4, 3177 / 4          | 40426 ms, 40377 ms, 37551 ms |
| growing conversation, turns 2–3                        | 2166 / 2150, 2188 / 2173          | 54 ms, 53 ms    | 3207 / 3172, 3237 / 3202              | 812 ms, 804 ms               |
| interleaved: the unrelated request itself              | 2444 / 28                         | 2175 ms         | 3176 / 4, 3176 / 4, 3176 / 4          | 40381 ms, 40604 ms, 39175 ms |
| interleaved: the original request repeated after it    | 2143 / 2136                       | 39 ms           | 3176 / 3171, 3176 / 3171, 3176 / 3171 | 215 ms, 205 ms, 159 ms       |
| retry after aborting mid-prefill (400 ms in, 5 s wait) | 2444 / 2443                       | 30 ms           | 3477 / 3472, 3477 / 2063, 3477 / 3472 | 148 ms, 18154 ms, 155 ms     |

The 2026-09-05 archived runs agree closely with the 2026-09-04 figures already in the table for identical-repeat and growing conversation, and are not re-tabulated for those rows: `llama3.2` read 1901 ms → 11 ms (cached 2142/2143) and turns 2–3 at 2166/2150 → 54 ms, 2188/2173 → 53 ms — the same token counts and millisecond figures as the row above, to the millisecond. `qwen3.8:27b-nvfp4` read 34456 ms → 143 ms (cached 3171/3176) for identical-repeat, and 3207/3172 → 768 ms, 3237/3202 → 737 ms for growing-conversation turns 2–3 — again the same token counts as the row above. The interleaved-repeat row's llama3.2 cell is unchanged from 2026-09-04 (2143/2136 at 39 ms); the archived run reads 2143/2136 at 45 ms, consistent. An unarchived same-day rerun of `qwen3.8:27b-nvfp4`'s interleaved phase (kept in `scratchpad`, not committed, run to check the archived reading) read `other` at 3176/4, 36601 ms and the repeat at 3176/3171, 165 ms — a fourth data point for each, consistent with the pattern below, reported here as corroboration rather than a counted point since it was not archived.

Two readings hold on both the second host and the second model, now confirmed by a third archived run apiece:

- **Identical repeat.** Cold prefill in the seconds against a warm repeat in tens of milliseconds, on both models, on every run measured (`llama3.2`: 2079 ms → 12 ms on 2026-09-04, 1901 ms → 11 ms archived; `qwen3.8:27b-nvfp4`: 37779 ms → 133 ms, confirmed at 158 ms on the 2026-09-04 repeat run and at 143 ms on the 2026-09-05 archived run).
- **A growing conversation pays for its new tokens only, on both models.** `qwen3.8:27b-nvfp4`'s per-turn prefill (812 ms, 804 ms on 2026-09-04; 430 ms, 802 ms, 812 ms on that day's repeat run; 397 ms, 768 ms, 737 ms for turns 1–3 on the 2026-09-05 archived run) runs higher than `llama3.2`'s (54 ms, 53 ms; 39 ms, 54 ms, 53 ms archived) but stays an order of magnitude below its own cold prefill on every run, consistent with the same behavior at a higher per-token cost.

Two readings are a confirmed miss on the large model, each reproduced across three archived runs (four counting the unarchived corroboration):

- **"Same system prompt, different question" reproduces the M5 pattern for `llama3.2` on this host, on every run, but not for `qwen3.8:27b-nvfp4`, on any run.** For `llama3.2`, this step reused nearly the full system prompt (87 ms on 2026-09-04, 42 ms on the 2026-09-05 archived run, ~2,136 of ~2,144 tokens cached both times). For `qwen3.8:27b-nvfp4`, the same step came back with `cached=4` of ~3,177 tokens and a prefill matching its own cold prefill on the two 2026-09-04 runs (40426 ms, 40377 ms) and again on the 2026-09-05 archived run (37551 ms) — three independent runs, all misses, within 8% of each other and nowhere near the 133–158 ms warm repeat measured for identical-repeat. A byte-identical system prompt with only the trailing question differing does not reuse the cache on this model, on this host, across three measurements.
- **Interleaving a genuinely unrelated request between two identical ones also misses, on `qwen3.8:27b-nvfp4`, on every run — but it does not evict the original sequence's cache slot.** The unrelated (`bravo`) prompt itself came back at `cached=4` of 3176 across all three archived runs (40381 ms, 40604 ms, 39175 ms), an expected near-cold prefill for a genuinely different system prompt. The original (`alpha`) request repeated immediately after it, though, came back warm every time — 3171 of 3176 cached, at 215 ms, 205 ms, and 159 ms across the three runs — matching identical-repeat's warm-repeat figures rather than the unrelated request's miss. `llama3.2` shows the same shape: the unrelated request misses (2444/28, 2175 ms, archived only) while the repeat afterward stays warm (2143/2136, 39 ms on 2026-09-04, 45 ms archived). This is the same pattern the M5 measurement's "cache survives interleaving" reading describes for `llama3.2`.

One reading is unstable on the large model:

- **Retry after an abort costs a warm repeat on `llama3.2` on every run** (30 ms on 2026-09-04, 27 ms on the 2026-09-05 archived run, against 29 ms on the M5), **but the same step on `qwen3.8:27b-nvfp4` remains unstable.** The two 2026-09-04 runs read 148 ms with 3472 of 3477 cached, then 18154 ms with 2063 of 3477 cached. The 2026-09-05 archived run reads 155 ms with 3472 of 3477 cached — close to the first reading, not the second. The same unarchived same-day rerun mentioned above also repeated this step and read 142 ms at 3472 cached, reported here as corroboration and not as a counted fourth point, since it was not archived. What has been observed is three readings near 148–155 ms and one at 18154 ms; that is not a rate, and one anomaly among a handful of readings is a real observation about variability rather than noise to average away. Every reading is partial reuse rather than a full cold prefill (0 cached) or a full warm repeat (3477 cached), and the amount reused and the cost still differ by roughly two orders of magnitude between the fastest and slowest reading recorded. A plausible reason — how much of the aborted call's prefill had completed server-side before the 400 ms abort landed varies run to run, and the retry only needs to redo what was not yet computed — is consistent with partial-but-variable reuse; it is not confirmed, since the probe does not observe server-side progress directly.

For `qwen3.8:27b-nvfp4`, a byte-identical repeat and a strictly-growing conversation reuse the cache on every run measured; a fresh request sharing the system prompt without extending an already-cached sequence misses entirely on every run measured; interleaving a genuinely unrelated request also misses entirely on every run measured, but does not evict the original sequence's cache slot, which stays warm afterward on every run measured; and a retry after an abort read close to a warm repeat on three of the four runs and far from it on one. Why the cross-question and unrelated-interleaved requests miss entirely, and why the retry's reuse is variable, is not established — the probe reports `prompt_eval_cached_count`, not the runner's cache-key logic or its server-side scheduling — so a model- or quantization-specific matching rule, a context-length effect past ~3,000 tokens, and a slot-count limit are each consistent with the miss counts, and abort-timing variance is consistent with the retry instability, but none of these is confirmed.

- Caveat: single-turn-scale replies, and the runner's retention limits and eviction policy under memory pressure, remain unprobed. Identical-repeat and growing-conversation reuse reproduced across two hosts (M5, M1 Max) and two models (`llama3.2:latest`, `qwen3.8:27b-nvfp4`) under the same Ollama 0.33.3, confirmed by two further repeat runs for the large model (2026-09-04, 2026-09-05 archived) — that part of the shape is not an artifact of one machine, a 2 GB model, or a single measurement. Reusing a shared system prompt across a different fresh question is not general runtime behavior: it held for `llama3.2` on both hosts and missed entirely, across three independent runs, for `qwen3.8:27b-nvfp4` on the M1 Max. The cache surviving an interleaved unrelated request is a fixed property across every run measured: the unrelated request itself always misses, but it never evicted the original sequence's slot, on either model, across three archived `qwen3.8:27b-nvfp4` runs and a fourth unarchived one. Retry-after-abort cost is not a fixed property of the runtime either: it matched the M5 figure for `llama3.2` on every run, but on `qwen3.8:27b-nvfp4` three of four readings clustered near 148–155 ms and one read 18154 ms. These figures are this runtime's behavior for two specific models on one host, not this runtime's cache behavior in general — and for the large model, one of the six readings is not stable across repeat runs of the same probe.
