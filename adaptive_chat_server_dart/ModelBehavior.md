...
### The cross-question miss follows the runner, not the model, and is bounded by the first divergence from a cached prefix
...
`qwen3-coder:30b` rules out model family: it is a Qwen build in the same size class as the two `nvfp4` builds, served by `llama-server`, and it reuses the cache the same way `llama3.2:latest` does. The split is runner, not family or size.
...
On both MLX-served builds, `fresh-1` costs the same as `cold` — a full prefill, within a few percent, whether or not an exact repeat ran in between — and `fresh-2` and `fresh-3` both reuse almost the whole prompt. `llama3.2:latest` shows no such split: every fresh question reuses the prompt, `fresh-1` included, in both arms.

**This resolves the cross-question miss as a property of the first divergence from a cached prefix on the MLX runner, not a property of the model, the quantization, or the prompt.** On the MLX runner, only the first request that diverges from a prompt's cached prefill pays the cost of a cold one; every later divergence from that same branch, in any order tried, reuses it. The earlier section's "no plain structural explanation" no longer holds — there is one, bounded to this runner — but the mechanism inside the runner that ties a restore to having visited a branch once is not read from Ollama's source, only inferred from these readings, and stays a hedge. What it does explain: the five-phase probe's `new-question` step sends exactly one fresh question per system prompt, so on the MLX runner it was always this same first-divergence event — which is why it read as missing on every run of that step, on both `qwen3.8:27b-nvfp4` and `qwen3.6:27b-coding-nvfp4`, without needing a per-model or per-quantization rule.
