...
- **The prompt-cache figures have never been taken with thinking on.** `prefill_cache_probe.dart` caps generation at `num_predict` 60 and sends `think: false`, which is what the chat server sends, so every figure in [the prompt-cache section](#prompt-cache-reuse-and-retry-cost-measured-with-prompt_eval_cached_count) describes a short non-thinking reply. Eight of the sixteen models on this host declare `thinking`, and with the flag off the reply is the answer rather than a reasoning preamble. What is untested is whether a thinking model's longer generation changes what the runner keeps: the cache figures are prefill-side and should not move, but the growing-conversation phase replays each reply, so a reply hundreds of tokens long rather than tens changes the prompt the next turn sends. A run with `think: true` and a `num_predict` large enough to finish a reply would settle it, and would need its own archives rather than a re-read of these.
...
| recurrent (inferred) | MLX            | a full cold prefill the first time, then warm (`qwen3.8:27b-nvfp4`, `qwen3.6:27b-coding-nvfp4`, `mvincig11/semif-qwen3.5-4b-mlx-4bit`) |
...
**The long retry is a timing effect, not an MLX one.** `qwen3.5:9b` retried with 3475 of 3479 cached and 76 ms of prefill inside an 8.1 s call, against a cold request of 9.6 to 11.3 s total. Its cold prefill, like the MLX builds', outlasts the probe's 0.4 s abort plus 5 s wait. `nemotron-3-nano:4b` (cold prefill about 5.8 to 6.4 s) retried in 2.2 s, and `gpt-oss:20b` (about 2.4 to 3.4 s) in 1.2 s. The wait is visible against a warm call rather than against the retry's own prefill, which never exceeds 175 ms: an exact repeat costs 0.12 to 1.27 s end to end by model, and the retry costs that plus whatever of the abandoned call the 0.4 s abort and 5 s pause did not cover. Four of the eleven `llama-server` models show no gap at all (`llama3.2:latest` 0.15 s against 0.12 s warm, `granite4.1:3b` 0.17 against 0.13, `gpt-oss:20b` 1.24 against 1.27, `nemotron-3-nano:30b` 0.27 against 0.14); the rest wait 1.9 s (`nemotron-3-nano:4b`) to 7.4 s (`qwen3.5:9b`), and the two MLX builds 33.5 s and 34.4 s. The retry's wall clock tracks how much of the abandoned prefill the wait did not cover, across both runners.
...
| --------------------------------------------------- | ------------ | --------------------------- | ------------------------------------ | ----------------------------------------------------- |
| `qwen2.5-coder:7b`                                  | standard     | 3174/5–16, 8.6–10.0 s       | 3167 on all 3, 106–137 ms            | 3167–3168 on all 12, 99–142 ms                        |
| `llama3-chatqa:8b`                                  | standard     | 2127/5–16, 4.7–6.2 s        | 2121 on all 3, 103–139 ms            | 2121 on all 12, 76–106 ms                             |
| `llama3-groq-tool-use:8b`                           | standard     | 2133/7–18, 4.7–6.1 s        | 2126 on all 3, 120–136 ms            | 2126 on all 12, 113–137 ms                            |
...
| recurrent (inferred) | MLX            | 3: `qwen3.8:27b-nvfp4`, `qwen3.6:27b-coding-nvfp4`, `mvincig11/semif-qwen3.5-4b-mlx-4bit`                                                    | a full cold prefill the first time, then warm                                                                    |

The retry readings follow the timing account on all eleven `llama-server` models too: a retry's total runs long wherever the cold prefill outlasts the 0.4 s abort plus 5 s wait (`qwen2.5-coder:7b`, 8.6–10.0 s cold, retried in 5.72 s; `granite4.1:8b`, 7.1–8.4 s cold, 4.76 s) and short where it does not (`granite4.1:3b`, 2.2–3.3 s cold, 0.18 s).
