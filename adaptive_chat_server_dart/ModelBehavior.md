...
**Measured directly on `qwen3.5:9b`, the comparability break is real.** `gguf_defaults_probe.dart` runs two arms at a fixed seed against the same prompt: `unpinned` sends only `temperature` and `seed` — what the shipped probes send today — and `pinned-historical` adds `top_k 40`, `top_p 0.9`, `presence_penalty 0` explicitly, the candidate-set defaults a pre-0.33.3 server applied regardless of the Modelfile. At `t=0.6`, `--samples 3`, the two arms produced **different replies in all 3 of 3 samples** — each arm's three samples hashed identically to each other and differently from the other arm — see [`gguf_defaults_probe.json`](tool/model_probes/results-m1max-64gb-ollama0333/qwen3.5_9b/gguf_defaults_probe.json). A `t=0` greedy run of the same two arms, meant as this design's control, also disagreed, which the design treats as a signal to investigate before trusting the sampled result rather than as the result itself. Isolating the three parameters at `t=0`: `presence_penalty 1.5` alone reproduced the `unpinned` arm's greedy output byte-for-byte, `presence_penalty 0` alone reproduced `pinned-historical`'s, and `top_k 40` + `top_p 0.9` alone matched `unpinned` — so the `t=0` divergence traces to `presence_penalty`, not to `top_k`/`top_p`. That narrows the earlier claim that `t=0` figures are shielded from this change: shielded from `top_k`/`top_p`, which constrain a sampling step greedy decoding skips, not from `presence_penalty`, which adjusts logits before the argmax is taken. Whether the mechanism is Ollama filling the unset options from `qwen3.5:9b`'s Modelfile, as the changelog entry implies, was not confirmed from server logs — the isolation above localizes the effect to `presence_penalty`, not the cause. One GGUF model on one host is evidence about this path, not a property of every GGUF build; figures sampled at `t=0.2`/`0.6` elsewhere in this file, and any `t=0` figure where `presence_penalty` was not pinned, should be read as not directly comparable across the 0.33.2-to-0.33.3 upgrade until re-measured.
...
| Pattern                                                 | `llama3.2:latest` prompt / cached | prefill         | `qwen3.8:27b-nvfp4` prompt / cached (two runs) | prefill (two runs)        |
| ------------------------------------------------------- | --------------------------------- | --------------- | ---------------------------------------------- | ------------------------- |
| identical request repeated                              | 2143 / 2142                       | 2079 ms → 12 ms | 3176 / 3171                                    | 37779 ms → 133 ms, 158 ms |
| same system prompt, different question                  | 2144 / 2136                       | 87 ms           | 3177 / 4, 3177 / 4                             | 40426 ms, 40377 ms        |
| growing conversation, turns 2–3                         | 2166 / 2150, 2188 / 2173          | 54 ms, 53 ms    | 3207 / 3172, 3237 / 3202                       | 812 ms, 804 ms            |
| identical request after an interleaved different prompt | 2143 / 2136                       | 39 ms           | 3176 / 4, 3176 / 4                             | 40381 ms, 40604 ms        |
| retry after aborting mid-prefill (400 ms in, 5 s wait)  | 2444 / 2443                       | 30 ms           | 3477 / 3472, 3477 / 2063                       | 148 ms, 18154 ms          |

Two readings hold on both the second host and the second model:

- **Identical repeat.** Cold prefill in the seconds against a warm repeat in tens of milliseconds, on both models (`llama3.2`: 2079 ms → 12 ms; `qwen3.8:27b-nvfp4`: 37779 ms → 133 ms, confirmed at 158 ms on the repeat run).
- **A growing conversation pays for its new tokens only, on both models.** `qwen3.8:27b-nvfp4`'s per-turn prefill (812 ms, 804 ms; 430 ms, 802 ms, 812 ms on the repeat run) runs higher than `llama3.2`'s (54 ms, 53 ms) but stays an order of magnitude below its own cold prefill on both runs, consistent with the same behavior at a higher per-token cost.

Two readings are a confirmed miss on the large model, reproduced across two independent runs on an idle machine:

- **"Same system prompt, different question" and "interleaved unrelated request" reproduce the M5 pattern for `llama3.2` on this host, but not for `qwen3.8:27b-nvfp4`, in either run.** For `llama3.2`, both steps reused nearly the full system prompt (87 ms and 39 ms). For `qwen3.8:27b-nvfp4`, both steps came back with `cached=4` of ~3,177 tokens and a prefill matching the cold prefill on the first run (40426 ms and 40381 ms) and again on the repeat run (40377 ms and 40604 ms) — within 1% of each other and nowhere near the 133–158 ms warm repeat measured one line above — despite a byte-identical system prompt, with only the trailing question, or an unrelated prompt entirely, differing. Two independent runs rule out a scheduling or contention artifact; this is a measured property of this model on this host, not a provisional reading.
- **Neither miss evicted the original sequence's cache slot, on either run.** `qwen3.8:27b-nvfp4`'s repeat of the original "identical" request, run after both misses, came back warm both times (3171 of 3176 cached, 215 ms then 205 ms).

One reading is unstable on the large model:

- **Retry after an abort costs a warm repeat on `llama3.2`, matching the M5's figure (30 ms here against 29 ms on the M5), but the same step on `qwen3.8:27b-nvfp4` gave two very different readings across the two runs** — 148 ms with 3472 of 3477 cached, then 18154 ms with 2063 of 3477 cached. Both are partial reuse rather than a full cold prefill (0 cached) or a full warm repeat (3477 cached), but the amount reused and the cost differ by roughly two orders of magnitude between runs. A plausible reason — how much of the aborted call's prefill had completed server-side before the 400 ms abort landed varies run to run, and the retry only needs to redo what was not yet computed — is consistent with partial-but-variable reuse; it is not confirmed, since the probe does not observe server-side progress directly.

For `qwen3.8:27b-nvfp4`, a byte-identical repeat and a strictly-growing conversation reuse the cache in both runs; a fresh request sharing the system prompt without extending an already-cached sequence misses entirely, also in both runs; and a retry after an abort reuses some but not all of the prompt, at a cost that varied by close to 100x between the two runs. Why the cross-question and interleaved requests miss entirely, and why the retry's reuse is variable, is not established — the probe reports `prompt_eval_cached_count`, not the runner's cache-key logic or its server-side scheduling — so a model- or quantization-specific matching rule, a context-length effect past ~3,000 tokens, and a slot-count limit are each consistent with the miss counts, and abort-timing variance is consistent with the retry instability, but none of these is confirmed.

- Caveat: single-turn-scale replies, and the runner's retention limits and eviction policy under memory pressure, remain unprobed. Identical-repeat and growing-conversation reuse reproduced across two hosts (M5, M1 Max) and two models (`llama3.2:latest`, `qwen3.8:27b-nvfp4`) under the same Ollama 0.33.3, each confirmed by a repeat run for the large model — that part of the shape is not an artifact of one machine, a 2 GB model, or a single measurement. Reusing a shared system prompt across different fresh questions is not general runtime behavior: it held for `llama3.2` on both hosts and missed entirely, in two independent runs, for `qwen3.8:27b-nvfp4` on the M1 Max. Retry-after-abort cost is not a fixed property of the runtime either: it matched the M5 figure for `llama3.2` but varied by roughly 100x between two runs on `qwen3.8:27b-nvfp4` (148 ms vs 18154 ms). These figures are this runtime's behavior for two specific models on one host, not this runtime's cache behavior in general — and for the large model, two of the five readings are not even stable across repeat runs of the same probe.
