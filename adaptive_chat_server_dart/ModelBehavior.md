...
**Ignored `format` under 0.32.14; honors it under 0.33.2.** Its canary read `ignored-harmlessly` on 2026-08-20, making `--json-format json|schema` inert for it; the 2026-09-01 sweep reads `honored` — see [the format canary](#not-a-card-test-the-format-canary) for the runtime change behind the flip. Whether the now-enforced constraint helps its permanent `Carousel`/`ColumnSet` invalid-JSON failures has not been measured; that A/B is the open follow-up the flip creates.
