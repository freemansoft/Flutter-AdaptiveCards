...
Every **With history** cell added on 2026-08-16 — all six `N/6` cells, not only `qwen2.5-coder:7b`'s `6/12` — was measured against `assets/card_system_prompt.txt` *after* Task 7's escape-hatch re-key had already been promoted. None of these numbers is a "before" baseline for a fix still to come; read them as the current, fixed-prompt state of each model.
