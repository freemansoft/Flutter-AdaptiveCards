...
**Retry-after-abort adds two corrections to the earlier reading.** Across this session's three `qwen3.8:27b-nvfp4` retry readings at 300 entries (five-, six-, and seven-phase archives) plus one at 200 entries, the retry read `cached=2063` on three of the four and `cached=3472` on the fourth — a repeat of the earlier section's instability, not a resolution of it. The recurring `2063` is notable on its own: it reads identical at two different total prompt sizes (2292 tokens at 200 entries, 3477 at 300), which a coincidence of prompt length cannot explain. A fixed restorable point somewhere in the runner's chunking is one explanation consistent with that; it is not confirmed. `qwen3.6:27b-coding-nvfp4`'s retry did not show this instability — `cached=3473` on all three readings this session, the model's retry behavior not measured before this session. `llama3.2:latest`'s retry read a full warm repeat on two of its three readings this session (`cached=2443`) and a partial one on the third (`cached=2076`, 796 ms) — the earlier section's "costs a warm repeat on `llama3.2` on every run" no longer holds without qualification; `llama-server`'s retry is close to deterministic here, not perfectly so. **`prompt_eval_duration` understates what the MLX retry cost.** The archives record `totalMs` for every call, and every retry on both MLX builds took 37.4 to 47.3 s of wall clock, about a full cold request, whether its prefill read 142 ms or 17.6 s; a warm identical repeat on the same builds totals 2.3 to 5.2 s. The 0.33.3 archive reads the same (142 ms prefill inside a 39,693 ms call), so the earlier section's 142–148 ms retries were fast prefills inside slow calls. That is consistent with the retry waiting behind the abandoned prefill, which the 5 s wait cannot cover on a ~36 s prefill; the probe does not observe server-side progress directly. `qwen3-coder:30b` also retried with a 20–22 ms prefill inside a 2.8–5.6 s call, against a ~0.3 s warm repeat. `llama3.2:latest`'s retries totaled 148–157 ms on its two warm readings.
...
**Both recurrent models re-process the last 1,025 tokens of the shared prefix on every new conversation, the first included, and never fall to a full cold prefill.** `llama-server` logs why. On the first new conversation it prints `forcing full prompt re-processing due to lack of cache data (likely due to SWA or hybrid/recurrent memory …)` for the tail, then `restored context checkpoint (… n_tokens = 2152 …)` on `qwen3.5:9b` and `n_tokens = 2454` on `nemotron-3-nano:4b`. The runner writes a context checkpoint about 1,024 tokens before the end of each prompt and one at the end (`created context checkpoint … n_tokens = 2152` and `3172`; the prefill itself proceeds in 1,024-token batches). A new conversation diverges about 10 tokens from the end, past the last checkpoint that still matches, so the runner resumes from the earlier one. The cost is about a third of a cold prefill, every time: 3.2 to 4.0 s against 8.7 to 11.0 s on `qwen3.5:9b`, about 2.0 s against 5.4 to 6.4 s on `nemotron-3-nano:4b`. An exact repeat and a growing conversation's turns 2 and 3 restore the end checkpoint and stay warm. The unrelated request reuses nothing, not even the instruction prefix (`cached=0`), since no checkpoint sits there.
...
`granite4.1` is standard attention by the log (`n_swa = 0`, no recurrent memory). Every standard-attention model restored the shared system prompt on every new conversation, the first included. Every Ollama-library recurrent build rolled back to the checkpoint 1,024 tokens before the prompt's end (2,153 of 3,177) on every new conversation. The unsloth build of the same Nemotron weights did so only when the call before was an exact repeat or an extension. After any other call it restored at 3,162 tokens instead, about 0.17 s against 1.75 s. Its prompt is one token longer (3,178) under its own chat template; why that changes which checkpoint survives is not read from the log here.
...
- Caveat: one seven-phase run per model for these eleven, one client, requests serial. The 1,025-token rollback is `llama-server`'s checkpoint placement at this prompt size; the log shows placement at the prompt's last 1,024-token batch boundary here, and a different prompt length would move it.
