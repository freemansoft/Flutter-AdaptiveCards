...
**The fit control reproduced on the 16 GB host to the token and to the case, before either side was calibrated.** The three models a 16 GB machine can hold under these windows were re-run there 2026-09-11, in [`m5-16gb-ollama0333-fitcontrol/`](tool/model_probes/context_fill_results/m5-16gb-ollama0333-fitcontrol). Every prompt count and every pass count is identical to the M1 Max reading, including `nemotron-3-nano:4b` carrying 46287 tokens inside a 65536 window on a host with a quarter the RAM. At `temperature 0` an identical pass count across two hosts is close to what a correct measurement should produce rather than a second opinion, but it does establish that neither ingestion nor coverage under a filled window depends on host memory here.
...
**The M1 Max column above is a superseded run, cited rather than archived.** Both sides of this comparison were taken with the fixed filler. `m5-16gb-ollama0333-fitcontrol/` still holds the M5 side; the M1 Max side was overwritten by the calibrated re-measurement in the table further up and survives in commit `9698553`. The comparison is reported as it was measured, since a cross-host reading taken under one condition on both hosts is what it establishes, and re-running only one side would not preserve it. What carries forward independently is that the Nemotron counts are unchanged by calibration: `nemotron-3-nano:4b` reads 6/25 on both hosts uncalibrated at 46287 tokens, and 6/25 again on the M1 Max calibrated at 48559.

**Latency under a filled window moves between hosts, and not in one direction.** Median s/call, M5 against M1 Max: `nemotron-3-nano:4b` 13.4 s against 7.0 s and `qwen2.5-coder:7b` 10.0 s against 5.1 s, both roughly 2x slower on the M5 and in the direction a narrower memory bus would predict for a prefill-dominated call; `qwen3.5:9b` runs the other way, 10.0 s against 14.1 s. One model inverting while two do not is not attributable from these runs: each figure is a single `--samples 1` measurement taken at a different point in its own sweep, and the position effect measured elsewhere in this file is 1.54x, large enough to cover the gap on its own.
...
- **Closed 2026-09-11: nothing drops a history message that fits.** This entry asked what discarded an oversized message when the runner had room for it. The [fit control](#a-filled-context-an-oversized-history-message-is-dropped-whole-and-a-real-one-costs-some-models-a-third-of-their-shapes) shows the premise was false: all six models ingested the filler once it fit, and every drop was a message 42426 tokens or larger under a 35851-token window. The probe's fixed `fillerCharsPerToken = 4.0` manufactured the gap by sizing history in characters. What stays open is narrower and dated from here. The probe still needs per-model calibration rather than a constant, since any run whose filler overflows measures overflow rather than capacity. The M5 archive predates `runnerContextLength`, so whether a memory-constrained host also allocates `min(requested, trained window)` is unmeasured. And the filled-context cost is measured at one fill size on one host, so the Nemotron family's loss of about a third of its shapes is a reading at roughly 42.5k tokens, not a curve.
