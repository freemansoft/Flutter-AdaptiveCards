...
That comparison alone cannot say whether a lower score comes from running past the allocation or from a full window, so the same 0.34.0 host also ran the calibrated fit control on both builds: about 48,500 tokens inside a 65536 allocation, reproducing the 0.33.3 control to the token and to the verdict. The remaining six models of that control followed on 2026-09-24 and reproduced the same way, so all eight have a 0.34.0 reading.
...
- **An attention-only build on the MLX runner is unmeasured.** [The batch section](#the-rollback-is-one-batch-the-mlx-miss-is-not-the-nvfp4-quantization-and-interleaved-conversations-pay-per-turn) separates the MLX runner's first-divergence cost from the `nvfp4` quantization and from the `qwen3_5` architecture, with a `nemotron_h` build reproducing the shape, but every MLX-served model measured is recurrent. The runner's prefix-cache source says attention (KV) layers restore at any position, which predicts no first-divergence cost for such a model; nothing has shown it. The runner at v0.34.0 registers `LlamaForCausalLM` and `Qwen3ForCausalLM`, so a `bfloat16` safetensors Llama 3.2 or Qwen3 imported with `ollama create --experimental`, as the Nemotron control was, is the experiment; `ollama show` should report the model on the MLX runner's side (safetensors, not GGUF) and the server log `starting mlx runner subprocess`.
...
- **The prompt-cache figures are all at one context size.** Every run in [the prompt-cache section](#prompt-cache-reuse-and-retry-cost-measured-with-prompt_eval_cached_count) requested `num_ctx` 8192, and nothing varied it. The `--entries` sweep in [the rollback section](#the-rollback-is-one-batch-the-mlx-miss-is-not-the-nvfp4-quantization-and-interleaved-conversations-pay-per-turn) puts the `llama-server` checkpoint a fixed distance from the end of the prompt rather than at a fixed offset in the window: the cached count stays exactly 1,024 below the prompt at every size from 1,549 to 4,812 tokens. On that reading the position is set by `n_batch` and the sequence end, so context-independence is a prediction from it rather than a measured result. One confounder is already recorded elsewhere in this file: the host sets `OLLAMA_CONTEXT_LENGTH=32768` and Ollama's allocated window can exceed the requested `num_ctx`, so these runs may have been allocated more than they asked for, and each archive records the request. The experiment is one model through the nine-phase probe on the same 300-entry glossary at `num_ctx` 8192 and again at 32768, reading `pos_min` and `n_tokens` from the server log beside `prompt_eval_cached_count` from each reply; `qwen3.5:9b` is the cheapest recurrent build already characterised at that size. Two further things stay untested at long contexts: whether the 32 checkpoint slots the log reports ever bind, since only two or three were ever created, and whether checkpoint size moves from the single logged `size = 50.251 MiB`.
...
| `nemotron-3-nano-mlx:4b-bf16`                       | recurrent      | MLX            | 3177/3173    | 19, 18    | 3177/3173, 65 ms  | 3478/3474, 69 ms/0.71 s   |

An immediate exact repeat leaves 1 to 5 tokens uncached on every model, and a growing-conversation turn re-evaluates 10 to 40. A repeat that follows the unrelated glossary is a different matter: it stays warm on the seven attention-only builds and on all four MLX builds, and costs the five recurrent `llama-server` builds their new-conversation price again. `gpt-oss:20b` falls back to a checkpoint at 1,162 tokens there, which it otherwise reaches on only one of its fifteen new conversations. The per-model cold, first-new-conversation and later-new-conversation figures are in the two tables above.

Across all seventeen measured models the new-conversation behavior sorts by memory type first and runner second:
...
| recurrent          | MLX            | 4: `qwen3.8:27b-nvfp4`, `qwen3.6:27b-coding-nvfp4`, `mvincig11/semif-qwen3.5-4b-mlx-4bit`, `nemotron-3-nano-mlx:4b-bf16`                     | a full cold prefill the first time, then warm                                                                    |
...
**The MLX miss is not the `qwen3_5` architecture either.** `nemotron-3-nano-mlx:4b-bf16` is `nvidia/NVIDIA-Nemotron-3-Nano-4B-BF16` from Hugging Face, a `nemotron_h` safetensors build at `bfloat16`, the same weights as `nemotron-3-nano:4b` measured on `llama-server` above. It was imported on 2026-09-26 with `ollama create --experimental`, which keeps the tensors as safetensors (the standard `ollama create` converts a safetensors directory to GGUF, which lands on `llama-server`), and Ollama serves it on the MLX runner (`starting mlx runner subprocess` in its archived log slice). The runner's model code makes it recurrent: [`x/models/nemotron_h/nemotron_h.go`](https://github.com/ollama/ollama/blob/v0.34.0/x/models/nemotron_h/nemotron_h.go) registers `NemotronHForCausalLM` and defines a `Mamba2` mixer for its recurrent layers, and the build's `config.json` interleaves them with attention (`hybrid_override_pattern` `M-M-M-MM-M-M*-M-M*-M-M-M*-M-M-MM*-MMM-M-M-`, `M` Mamba, `*` attention). Through the nine-phase probe (`t=0`, `num_ctx` 8192, 300 entries, `think: false`, one model resident) it reproduces the MLX shape: a cold first request (3177/0, 3.94 s prefill), a warm exact repeat (3177/3173, 66 ms), a cold first new conversation (3178/5, 4.90 s; the log reads `matched=3167 cached=5`, the `qwen3_5` builds' signature), and warm ones after. In the first-divergence phase each synthetic prompt costs one full prefill (3177/16, 4.62 to 4.77 s) with or without an exact repeat before it, then reuses 3,167. Two interleaved conversations re-evaluate 11 to 20 tokens a turn and a second branch 4 to 11, where the same weights on `llama-server` re-process 1,021 to 1,058 every turn and 957 on every second-branch call. Two architectures, three quantizations, one runner, one shape; the same Nemotron weights on the other runner show that runner's rollback. The first-divergence cost is the MLX runner's snapshot policy. Its retry read 3478/3474 in 0.71 s: a cold prefill of about 4.7 s ends inside the probe's 5.4 s wait, which is the timing account above. Archive: `results-m1max-64gb-ollama0340/nemotron-3-nano-mlx_4b-bf16/`.

Two things the control does not hold constant. The two Nemotron builds tokenize the glossary differently, 3,478 tokens through the GGUF's tokenizer against 3,177 through the safetensors `tokenizer.json`, so the weights are shared and the tokenizer is not. And the import needed one correction, recorded here because it is not in the upstream files: the repo's `generation_config.json` names token 2 (`</s>`) as the end token while its chat template closes every turn with `<|im_end|>` (token 11), and the MLX runner reads its end tokens from that file (`Tokenizer.IsEOS` in [`pipeline.go`](https://github.com/ollama/ollama/blob/v0.34.0/x/mlxrunner/pipeline.go)), so the first import ran every reply to the `num_predict` cap emitting `<|im_end|>` (`done_reason: length`, 60 tokens for `Paris.`). A `PARAMETER stop "<|im_end|>"` line in the Modelfile was not applied on that runner. Setting `eos_token_id` to `[2, 11]` in the copied `generation_config.json` before `ollama create` fixed it (`done_reason: stop`, 2 tokens). The Modelfile otherwise mirrors the library build, `RENDERER nemotron-3-nano` and `PARSER nemotron-3-nano`; the library's GGUF carries the right end token in its own metadata.

What remains unmeasured is an attention-only build on the MLX runner, which would confirm the source reading that sliceable KV layers restore at any position and so pay no first-divergence cost. Two earlier attempts failed and stay recorded: `pd95/gptoss-mlx:20b-mxfp4`, a safetensors MXFP4 build of `gpt-oss`, is routed to the MLX runner and refused, `mlx runner failed: Error: unsupported architecture: GptOssForCausalLM`; an MLX 4-bit Llama from `mlx-community` does not import, `ollama create` rejecting its packed tensors with `unknown data type: U32`. The runner at v0.34.0 registers `LlamaForCausalLM` and `Qwen3ForCausalLM`, so a `bfloat16` safetensors Llama or Qwen3 imported the same way as the Nemotron build is the remaining control.
...
- Caveat: this closed the cross-question miss's cause as far as this probe could establish it, with runner, quantization and architecture still confounded at the time. The quantization was ruled out and the runner's logic read from source later the same week, and the architecture was separated on 2026-09-26 with a `nemotron_h` build on the same runner; all three are in the batch section above.
