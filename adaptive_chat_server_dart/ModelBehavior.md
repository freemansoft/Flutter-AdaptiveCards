...
- **A history message that does not fit is dropped whole, not trimmed, and nothing says so.** Handed roughly 29.5k tokens of history under a requested 35851-token window, five of eight models evaluated only the system prompt and question, between 3819 and 4374 tokens. Two of the five have windows too short to hold it; the other three had the room. The reply reads as a normal answer to a question asked with no history, so the loss is invisible unless `prompt_eval_count` is checked. It reproduces across a 16 GB and a 64 GB host, which rules out available memory as the cause. See [a filled context](#a-filled-context-five-of-eight-models-drop-the-filler-whole-and-two-hosts-agree).
...
**The trained window explains two of the five drops.** `llama3-chatqa:8b` and `llama3-groq-tool-use:8b` are 8192-token models being handed roughly 29.5k, so the message cannot fit under any policy, and dropping it whole rather than trimming it is a choice about how it fails rather than whether. The other three have room. `qwen3.5:9b` and `nemotron-3-nano:4b` carry 262144-token windows, and `qwen2.5-coder:7b`'s 32768 is short of the requested 35851 but comfortably longer than the roughly 29.5k the prompt actually reaches. For those three the message was dropped with the window to hold it, and no mechanism here is established. A per-model or per-architecture history policy, a safety margin applied against the request rather than the prompt, and a runner clamp below the trained window are each consistent with the readings, and none is confirmed.

Three limits on the above. `summary.numCtx` records the value the probe **requested**, and both hosts record the same 35851, so it is not evidence about what the runner allocated; the only direct observation on this point is a single mid-sweep `ollama ps` reading of `granite4.1:8b` on the M1 Max at `CONTEXT 35851`, which is one model at one moment and does not cover the five that drop. The probe captures `/api/ps` once per model as of 2026-09-11 and archives `runnerContextLength` beside the requested `numCtx`, but both runs above predate that field, so settling this means re-running the sweep on both hosts rather than re-reading what is already recorded. Both runs are `--samples 1`, against the `--samples 2` most of this file carries. And the pass column is not like-for-like with [the shape-coverage table](#shape-coverage--all-fifteen-models-as-shipped), which was measured on a different runtime with two prose turns as history rather than a filler block, so read it within this section rather than against that one.
...
- **Why three models with room to spare still drop the filler, and what the runner actually allocated.** [The filled-context runs](#a-filled-context-five-of-eight-models-drop-the-filler-whole-and-two-hosts-agree) establish that the drop is whole-message, silent, and not caused by host memory. They do not establish the mechanism. `qwen3.5:9b` and `nemotron-3-nano:4b` carry 262144-token windows and `qwen2.5-coder:7b` carries 32768, all longer than the roughly 29.5k the prompt reaches, and all three discard it anyway. The two archived runs record the `num_ctx` the probe requested rather than what the runner allocated, so the clamping seen on the M5 via `/api/ps` is neither confirmed nor excluded for these models; the one direct reading on the M1 Max, `granite4.1:8b` at `CONTEXT 35851`, covers a model that kept the filler. The probe now takes that reading itself, recording `runnerContextLength` and `runnerContextClamped` from a single `/api/ps` call once the first case has loaded the runner. What remains is a re-run on both hosts, since the field cannot be backfilled into an existing archive.
