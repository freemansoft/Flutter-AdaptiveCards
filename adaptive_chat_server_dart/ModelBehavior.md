...
**The drop is whole-message, not truncation.** A model that trimmed the filler to fit would report a prompt count near its window. The five report the system prompt and question alone, between 3819 and 4374 tokens against the roughly 29.5k actually sent, which is the filler message discarded in full. Nothing errors and nothing warns, and the model answers the question as if no history had been supplied. This is the same class of silent loss as the truncation described under [measurement lessons](#measurement-lessons), reached by a different route: there the prompt exceeded `num_ctx` and was cut, here a message that would have fit the requested window was removed before the runner saw it.

**Host memory is not the mechanism.** The M5 run left open whether a 16 GB machine was too tight to hold the filler, since Ollama's own `/api/ps` had shown a runner clamped below the requested context on that host. The M1 Max has four times the RAM and the same runtime version, and it reproduces the M5 result: the same five models drop, the same three keep, and the pass counts agree on six of the eight rows. Whatever removes the message, available memory is not what selects for it.
...
**Allocation follows one rule, measured over twenty-six runs without a counterexample.** The M1 Max sweep was re-run 2026-09-11 with the probe reading `/api/ps` once per model, so the allocated context is measured rather than inferred. Across all eight models the runner allocated **`min(requested, trained window)`**: every clamp lands exactly on that model's own `n_ctx_train`, none at an intermediate value, and no model got less than its window could hold. The eight pass counts and eight token counts reproduced the first run exactly, so this is the same measurement with one field added, not a second reading that has to be reconciled with the first. A later sweep of the six models too large for a 16 GB host extends the rule to twenty runs: every one requested 35851 against a window of 131072 or more and received exactly 35851.

| Model                     | Trained window | Allocated | Clamped | Filler  |
| ------------------------- | -------------- | --------- | ------- | ------- |
| `llama3.2:latest`         | 131072         | 35851     | no      | kept    |
| `granite4.1:8b`           | 131072         | 35851     | no      | kept    |
| `granite4.1:3b`           | 131072         | 35851     | no      | kept    |
| `nemotron-3-nano:4b`      | 262144         | 35851     | no      | dropped |
| `qwen3.5:9b`              | 262144         | 35851     | no      | dropped |
| `qwen2.5-coder:7b`        | 32768          | 32768     | yes     | dropped |
| `llama3-groq-tool-use:8b` | 8192           | 8192      | yes     | dropped |
| `llama3-chatqa:8b`        | 8192           | 8192      | yes     | dropped |

**The rule is the same on a 16 GB host, which is what rules memory out rather than leaving it inferred.** Three of the M5 models were re-measured 2026-09-11 with the same instrumentation, across both the fixed filler and the fit control, adding six readings for twenty-six in total. Each matches its M1 Max counterpart exactly: `qwen2.5-coder:7b` clamped to 32768 under a 35851 request on both hosts, and `nemotron-3-nano:4b` and `qwen3.5:9b` allocated the full 65536 they asked for on both. A 16 GB machine allocating a 64k window is the reading that settles it — the earlier suspicion that a clamp was a memory ceiling does not survive, since the one clamp observed lands on `qwen2.5-coder:7b`'s own 32768 trained window and reads identically with four times the RAM.

| Model                | Requested | M5 16GB allocated | M1 Max 64GB allocated | Trained window |
| -------------------- | --------- | ----------------- | --------------------- | -------------- |
| `nemotron-3-nano:4b` | 35851     | 35851             | 35851                 | 262144         |
| `qwen3.5:9b`         | 35851     | 35851             | 35851                 | 262144         |
| `qwen2.5-coder:7b`   | 35851     | **32768**         | **32768**             | 32768          |
| `nemotron-3-nano:4b` | 65536     | 65536             | 65536                 | 262144         |
| `qwen3.5:9b`         | 65536     | 65536             | 65536                 | 262144         |
| `qwen2.5-coder:7b`   | 32768     | 32768             | 32768                 | 32768          |
...
The control is [`m1max-64gb-ollama0333-fitcontrol/`](tool/model_probes/context_fill_results/m1max-64gb-ollama0333-fitcontrol): the same six models, each given a filler that fits the window it is actually allocated. **All six ingested it.** The `was` columns are the fixed-filler runs under a 35851-token window, and every one of those prompts is 42426 tokens or larger in that model's own tokenization, so none of them ever fit.

| Model                        | Prompt tokens | Was  | Pass      | Was   |
| ---------------------------- | ------------- | ---- | --------- | ----- |
| `qwen3.5:9b`                 | 42540         | 3965 | 19/25     | 18/25 |
| `qwen3-coder:30b`            | 42426         | 3850 | 18/25     | 18/25 |
| `qwen2.5-coder:7b`           | 19974         | 3850 | 21/25     | 22/25 |
| `nemotron-3.5-lightning:30b` | 42603         | 4027 | **13/25** | 20/25 |
| `nemotron-3-nano:30b`        | 42602         | 4027 | **12/25** | 17/25 |
| `nemotron-3-nano:4b`         | 46287         | 4374 | **6/25**  | 8/25  |

`qwen2.5-coder:7b` is the row a larger ceiling could not fix. Its trained window is 32768, and under `min(requested, trained window)` a bigger `--num-ctx` changes nothing, so it got a smaller filler instead: 12000 tokens rather than 28000, hence its lower prompt count. The comparison still holds, since the question is whether a message that fits is kept.

**The fit control reproduces on the 16 GB host to the token and to the case.** The three models a 16 GB machine can hold under these windows were re-run there 2026-09-11, in [`m5-16gb-ollama0333-fitcontrol/`](tool/model_probes/context_fill_results/m5-16gb-ollama0333-fitcontrol). Every prompt count and every pass count is identical to the M1 Max reading, including `nemotron-3-nano:4b` carrying 46287 tokens inside a 65536 window on a host with a quarter the RAM. At `temperature 0` an identical pass count across two hosts is close to what a correct measurement should produce rather than a second opinion, but it does establish that neither ingestion nor coverage under a filled window depends on host memory here.
...
**What a genuinely filled context costs is strongly model-dependent, and this is the first measurement of it here.** Every earlier figure for these models was taken with the history discarded, which measures the unfilled task. With roughly 42.5k tokens actually in the window, `qwen3-coder:30b` does not move at all and `qwen3.5:9b` gains a case, both inside the noise of `--samples 1`. The Nemotron models lose about a third of their shapes: `nemotron-3.5-lightning:30b` 20 to 13, `nemotron-3-nano:30b` 17 to 12. `nemotron-3-nano:4b` goes 8 to 6, the smallest model carrying the largest real prompt. Capacity under a full window is not predicted by score on an empty one.

Three limits on the above. The allocation reading covers **three of the eight models on the M5** — `nemotron-3-nano:4b`, `qwen3.5:9b` and `qwen2.5-coder:7b`, re-measured 2026-09-11 — while the other five M5 files predate `runnerContextLength` and cannot be backfilled, so the rule is established on that host for the three models re-run rather than for its whole column. Every run in this section is `--samples 1`, against the `--samples 2` most of this file carries, so a one-case difference is noise and only the Nemotron drops of five to seven cases are large enough to read. And no pass column here is like-for-like with [the shape-coverage table](#shape-coverage--all-fifteen-models-as-shipped), which was measured on a different runtime with two prose turns as history rather than a filler block, so read these within this section rather than against that one.
