...
### Two readings hold on both models; one is a confirmed miss on the large model; two are unstable on it
...
One reading is a confirmed miss on the large model, reproduced across three independent runs:
...
- **Neither of the 2026-09-04 misses evicted the original sequence's cache slot, on either 2026-09-04 run.** `qwen3.8:27b-nvfp4`'s repeat of the original "identical" request, run after both misses that session, came back warm both times (3171 of 3176 cached, 215 ms then 205 ms). This was a separate check beyond the interleaved-repeat step in the table above; `prefill_cache_probe.dart`'s current five phases do not reproduce it, so it is not re-confirmed here.

Two readings are unstable on the large model:

- **"Identical request after an interleaved different prompt" does not reproduce as a miss a third time.** Both 2026-09-04 runs read `cached=4` of 3176 for this step — a near-cold prefill, 40381 ms and 40604 ms — which is why the prior text called it a confirmed measured property. The 2026-09-05 archived run reads the opposite: `cached=3171` of 3176, 159 ms, a warm repeat matching the identical-repeat and growing-conversation pattern. An unarchived same-day rerun (kept in `scratchpad`, not committed, run to check whether the archived reading was a one-off) read `cached=3171` again, 165 ms. Two runs on one day read a miss; two runs on a later day read a hit. Four points on two days do not say which behavior is more common — they say the reading is not a stable property of this model on this host, contrary to what the two 2026-09-04 runs alone suggested.
...
For `qwen3.8:27b-nvfp4`, a byte-identical repeat and a strictly-growing conversation reuse the cache on every run measured; a fresh request sharing the system prompt without extending an already-cached sequence misses entirely on all three runs measured; the identical request after an interleaved different prompt missed on both 2026-09-04 runs and hit on both 2026-09-05 runs; and a retry after an abort read close to a warm repeat on three of the four runs and far from it on one. Why the cross-question request misses entirely, why the interleaved-repeat reading changed between measurement sessions, and why the retry's reuse is variable, is not established — the probe reports `prompt_eval_cached_count`, not the runner's cache-key logic or its server-side scheduling — so a model- or quantization-specific matching rule, a context-length effect past ~3,000 tokens, a slot-count limit, and some session- or time-dependent runner state are each consistent with the miss counts and the interleaved reading's flip, and abort-timing variance is consistent with the retry instability, but none of these is confirmed.

- Caveat: single-turn-scale replies, and the runner's retention limits and eviction policy under memory pressure, remain unprobed. Identical-repeat and growing-conversation reuse reproduced across two hosts (M5, M1 Max) and two models (`llama3.2:latest`, `qwen3.8:27b-nvfp4`) under the same Ollama 0.33.3, confirmed by two further repeat runs for the large model (2026-09-04, 2026-09-05 archived) — that part of the shape is not an artifact of one machine, a 2 GB model, or a single measurement. Reusing a shared system prompt across a different fresh question is not general runtime behavior: it held for `llama3.2` on both hosts and missed entirely, across three independent runs, for `qwen3.8:27b-nvfp4` on the M1 Max. Whether the cache survives an interleaved unrelated request is not a fixed property of the runtime either: for `qwen3.8:27b-nvfp4` it missed on both 2026-09-04 runs and hit on both 2026-09-05 runs, so on the evidence so far this reading tracks the measurement session at least as much as it tracks the model. Retry-after-abort cost is not a fixed property of the runtime either: it matched the M5 figure for `llama3.2` on every run, but on `qwen3.8:27b-nvfp4` three of four readings clustered near 148–155 ms and one read 18154 ms. These figures are this runtime's behavior for two specific models on one host, not this runtime's cache behavior in general — and for the large model, two of the five readings are not stable across repeat runs of the same probe.
