...
**Re-measured under Ollama 0.33.3, both `nvfp4` verdicts held** — `qwen3.6:27b-coding-nvfp4` on 2026-09-04 and `qwen3.8:27b-nvfp4` on 2026-09-07.\*\* `qwen3.8:27b-nvfp4` and `qwen3.6:27b-coding-nvfp4` both still read `honored` — see [`results-m1max-64gb-ollama0333/`](tool/model_probes/results-m1max-64gb-ollama0333). The other two verdicts were not re-run this pass. The probe scores all six of `gpt-oss:20b`'s calls `PASS`, because an empty reply is not a _broken card_ and the pass rule only fails broken cards: that is the judging rule working as designed for card quality and reading misleadingly here, so the verdict line rather than the PASS is this probe's output. Ollama 0.33.3's changelog also adds "Honor GGUF model defined default parameters." No `format` verdict has been taken under 0.34.0 for either `nvfp4` build or for `gpt-oss:20b`, although the canary is a property of model **and** runtime and is meant to be re-run after every upgrade; `granite4.1:3b` is the only model with a 0.34.0 verdict.
...
- **Closed 2026-09-07: the M1 Max re-run owed by the `Input.Rating` palette
  edit landed, and `check_results.dart` reports 0 fatal findings.** The edit moved
  ...
  closed with a `HISTORICAL.md` marker; `results-m1max-64gb-ollama0333/` was the
