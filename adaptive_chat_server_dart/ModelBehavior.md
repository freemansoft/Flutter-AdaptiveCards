...
- **A history message that does not fit is dropped whole, not trimmed, and nothing says so.** Handed roughly 29.5k tokens of history under a requested 35851-token window, five of eight models evaluated only the system prompt and question, between 3819 and 4374 tokens. Ollama allocates `min(requested, trained window)`, so two of the five were clamped to a window too short to hold it. The other three were not: two were allocated every token they asked for and discarded the message regardless. The reply reads as a normal answer to a question asked with no history, so the loss is invisible unless `prompt_eval_count` is checked. It reproduces across a 16 GB and a 64 GB host, which rules out available memory as the cause. See [a filled context](#a-filled-context-five-of-eight-models-drop-the-filler-whole-and-two-hosts-agree).
...
### A filled context: five of eight models drop the filler whole, and two hosts agree
...
`nemotron-3-nano:4b` and `qwen3.5:9b` are the rows that settle it. Both were allocated the full 35851 they asked for, both had room for the roughly 29.5k the prompt reaches, and both discarded the filler anyway, reporting 4374 and 3965 tokens. Neither memory nor a clamped window can account for that, which leaves something dropping an oversized history message before the runner sees it. Where clamping does account for the behavior is `llama3-chatqa:8b` and `llama3-groq-tool-use:8b`, clamped to 8192 and handed roughly 29.5k, which no policy could have fitted.

`qwen2.5-coder:7b` is the one row this does not resolve. It clamped to 32768, and roughly 29.5k fits inside that, but 29.5k is what the **other** models' tokenizers made of the filler text. Its own tokenization of that text is unmeasured, so whether it had room is not established, and it should not be counted either way.

Three limits on the above. The allocation reading is **M1 Max only**: the M5 archive predates `runnerContextLength` and the field cannot be backfilled, so whether that host clamps the same way is unmeasured, and the M5 is the host where a clamp could also be a memory effect rather than the trained-window rule. Both runs are `--samples 1`, against the `--samples 2` most of this file carries. And the pass column is not like-for-like with [the shape-coverage table](#shape-coverage--all-fifteen-models-as-shipped), which was measured on a different runtime with two prose turns as history rather than a filler block, so read it within this section rather than against that one.
...
- **What drops an oversized history message when the runner had room for it.** [The filled-context runs](#a-filled-context-five-of-eight-models-drop-the-filler-whole-and-two-hosts-agree) have ruled out the two candidates that were easy to test. Host memory is excluded, since a 64 GB machine reproduces a 16 GB one exactly. Clamping is excluded for `nemotron-3-nano:4b` and `qwen3.5:9b`, which the 2026-09-11 M1 Max re-run measured at the full 35851 they requested, with room for the roughly 29.5k prompt, discarding the filler anyway. What is left is the layer between the request and the runner, and nothing here identifies it: Ollama's own history handling, a per-architecture template that drops rather than truncates an oversized message, and a margin applied against the request are each consistent with the readings. Two narrower gaps sit beside it. `qwen2.5-coder:7b` clamped to 32768 and its own tokenization of the filler is unmeasured, so whether it had room is unknown. And the allocation reading is M1 Max only, since the M5 archive predates `runnerContextLength`; re-running that host would show whether a memory-constrained machine still follows `min(requested, trained window)`.
