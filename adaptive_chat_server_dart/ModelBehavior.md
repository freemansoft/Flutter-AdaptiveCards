...
- **A history message that does not fit is dropped whole, not trimmed, and nothing says so.** Handed roughly 29.5k tokens of history under a requested 35851-token window, five of eight models evaluated only the system prompt and question, between 3819 and 4374 tokens. Ollama allocates `min(requested, trained window)`, and a message past that window is removed rather than trimmed. Sizing history in characters is what hides this: the same text tokenizes at 4.30 chars/token on llama, granite and gpt-oss builds and at 2.74 to 2.99 on Qwen and Nemotron ones, so a history block that fits one model overflows another by half. The reply reads as a normal answer to a question asked with no history, so the loss is invisible unless `prompt_eval_count` is checked, and it reproduces identically on a 16 GB and a 64 GB host. See [a filled context](#a-filled-context-an-oversized-history-message-is-dropped-whole-and-a-real-one-costs-some-models-a-third-of-their-shapes).
...
**The M1 Max column is a mixed harness.** [`results-m1max-64gb-ollama0332/`](tool/model_probes/results-m1max-64gb-ollama0332) holds 13 models measured **before runner eviction** (the 2026-09-01 sweep) and 2 measured **after runner eviction** (`llama3.2:latest` and `granite4.1:3b`, re-run 2026-09-02); the harness change is described in [the cascade section](#stalls-are-a-queueing-cascade-not-a-runtime-difference). Eviction is a no-op unless a call times out; 11 of the 13 recorded zero stalls, and the other two (`hf.co/unsloth/…` at 3, `nemotron-3-nano:4b` at 1) match their 0.32.14 counts, so the split does not touch their rows. `granite4.1:3b`'s sweep and stall cells (124.0 min, 52) are its after-eviction run, whose stall positions still carry the cascade signature; they are not model figures, and only its median should be read.
...
[`context_fill_probe.dart`](tool/model_probes/context_fill_probe.dart) runs the same 25 cases `shape_ab.dart` runs, but prepends a large deterministic filler block as history in place of the two prose turns. It asks whether a model still answers in cards once most of its window is spoken for. It is a standalone diagnostic rather than one of the seven sweep probes: `check_results.dart` does not scan its output, and its archives live under `context_fill_results/` rather than a `results-*` directory.

Two runs at `--fill-tokens 28000`, both under Ollama 0.33.3 at `--samples 1`, over the eight models a 16 GB host can hold. [`m5-16gb-ollama0333-fill28000/`](tool/model_probes/context_fill_results/m5-16gb-ollama0333-fill28000) was measured 2026-09-10 on the Apple M5 / 16 GB and re-measured there 2026-09-11 to add the allocation reading, reproducing every pass and token count; [`m1max-64gb-ollama0333-fill28000/`](tool/model_probes/context_fill_results/m1max-64gb-ollama0333-fill28000) was measured 2026-09-11 on the Apple M1 Max / 64 GB. Both requested `num_ctx` 35851, which is the 28000-token fill target plus the card system prompt's own 3755-token estimate. **Tokens** below is Ollama's `prompt_eval_count`, the floor across the 25 cases, and it is the whole measurement: a model that ingested the filler reports roughly 29.5k, and a model that did not reports the system prompt and question alone. Both hosts get their own column for it and for the pass count, so the memory comparison can be read off the table rather than taken on trust.

| Model                     | Trained window | M5 16GB tokens | M1 Max 64GB tokens | M5 pass | M1 Max pass | Filler  |
| ------------------------- | -------------- | -------------- | ------------------ | ------- | ----------- | ------- |
| `llama3.2:latest`         | 131072         | 29546          | 29546              | 8/25    | 9/25        | kept    |
| `granite4.1:8b`           | 131072         | 29536          | 29536              | 8/25    | 8/25        | kept    |
| `granite4.1:3b`           | 131072         | 29536          | 29536              | 3/25    | 3/25        | kept    |
| `qwen2.5-coder:7b`        | 32768          | 3850           | 3850               | 22/25   | 22/25       | dropped |
| `qwen3.5:9b`              | 262144         | 3965           | 3965               | 17/25   | 18/25       | dropped |
| `nemotron-3-nano:4b`      | 262144         | 4374           | 4374               | 8/25    | 8/25        | dropped |
| `llama3-groq-tool-use:8b` | 8192           | 3825           | 3825               | 10/25   | 10/25       | dropped |
| `llama3-chatqa:8b`        | 8192           | 3819           | 3819               | 2/25    | 2/25        | dropped |

The **Filler** verdict is one column because it is identical on both hosts, which is the finding rather than a compression of the table: no model changes its behavior with four times the memory available.

**The drop is whole-message, not truncation.** A model that trimmed the filler to fit would report a prompt count near its window. The five report the system prompt and question alone, between 3819 and 4374 tokens, against a filler that tokenizes to between 42426 and 46287 tokens for them: the message discarded in full rather than cut down. Nothing errors and nothing warns, and the model answers the question as if no history had been supplied. This is the same class of silent loss as the truncation described under [measurement lessons](#measurement-lessons), and the two differ in what is lost rather than in whether the window was exceeded. Both prompts exceeded it. There the prompt was cut and the model saw the front of it; here the whole message was removed before the runner saw any of it.

**The two hosts' prompt counts match to the token, which corroborates nothing.** `prompt_eval_count` counts tokenized input, and the filler, the system prompt, and the 25 cases are byte-identical between the runs, so an exact match is what a correct measurement looks like rather than independent confirmation of a behavior. What the second host does add is the pass column, which is generated text and did move: `llama3.2:latest` 8 to 9 and `qwen3.5:9b` 17 to 18, one case each at `--samples 1`, which is noise on the file's usual reading.

**Allocation follows one rule, measured over thirty-one runs on two hosts without a counterexample.** Both sweeps were re-run 2026-09-11 with the probe reading `/api/ps` once per model, so the allocated context is measured rather than inferred. The runner allocated **`min(requested, trained window)`** every time: each clamp lands exactly on that model's own `n_ctx_train`, none at an intermediate value, and no model got less than its window could hold. Every pass count and token count reproduced its earlier run exactly, on both hosts, so this is the same measurement with one field added rather than a second reading to reconcile with the first. The six models too large for a 16 GB host extend the rule on the M1 Max: every one requested 35851 against a window of 131072 or more and received exactly 35851.

| Model                     | Trained window | M5 16GB allocated | M1 Max 64GB allocated | Clamped | Filler  |
| ------------------------- | -------------- | ----------------- | --------------------- | ------- | ------- |
| `llama3.2:latest`         | 131072         | 35851             | 35851                 | no      | kept    |
| `granite4.1:8b`           | 131072         | 35851             | 35851                 | no      | kept    |
| `granite4.1:3b`           | 131072         | 35851             | 35851                 | no      | kept    |
| `nemotron-3-nano:4b`      | 262144         | 35851             | 35851                 | no      | dropped |
| `qwen3.5:9b`              | 262144         | 35851             | 35851                 | no      | dropped |
| `qwen2.5-coder:7b`        | 32768          | **32768**         | **32768**             | yes     | dropped |
| `llama3-groq-tool-use:8b` | 8192           | **8192**          | **8192**              | yes     | dropped |
| `llama3-chatqa:8b`        | 8192           | **8192**          | **8192**              | yes     | dropped |

**Host memory does not enter into it, which is measured on both hosts rather than inferred from one.** All eight models were re-measured on the M5 across 2026-09-11, and every allocation matches its M1 Max counterpart exactly — the three clamps included, each landing on its own trained window rather than on anything the host could explain. The fit control adds the reading that settles it: a 16 GB machine allocated the full **65536** window requested for `nemotron-3-nano:4b` and `qwen3.5:9b`, and 32768 for `qwen2.5-coder:7b`, identical to the 64 GB host. The earlier suspicion that a clamp was a memory ceiling does not survive a 16 GB box allocating a 64k window on request.

**Every one of those drops was a message that did not fit. The apparent mystery was an artifact of this probe.** `buildFillerText` sizes the filler in **characters**, at `fillerCharsPerToken = 4.0`. Measured against the same 127,020 characters, that constant holds for three unrelated families and for nothing else: `llama3.2:latest` 29546, `granite4.1:8b` 29536 and `gpt-oss:20b` 29616 tokens, all 4.29 to 4.30 chars/token, against `qwen3.8:27b-nvfp4` at 42542 and `qwen3.6:27b-coding-nvfp4` at 42538 (2.99), and `nemotron-3-nano:4b` at 46287 (2.74). The filler is `filler-term123 means concept456.`, which is digit-heavy, and tokenizers split digits very differently. Wherever the constant under-counts, the probe sized a `num_ctx` too small for its own filler, the prompt overflowed, and the archive recorded a model discarding history it appeared to have room for.

The control is [`m1max-64gb-ollama0333-fitcontrol-calibrated/`](tool/model_probes/context_fill_results/m1max-64gb-ollama0333-fitcontrol-calibrated), re-measured 2026-09-12 with calibration on so that `--fill-tokens` means what it says on every tokenizer: eight models, each given a filler that fits the window it is actually allocated. **All eight ingested it.** **chars/token** is what calibration measured for that model against the 4.0 the fixed filler assumed, and it is the whole explanation for everything above. The **Was** columns are the fixed-filler sweep under a 35851-token window: six of those prompts arrived as 3850 to 4374 tokens, the system prompt and question alone after the history was dropped, while the two `nvfp4` rows kept theirs and are the exception below.

| Model                        | chars/token | Prompt tokens | Was   | Pass      | Was   |
| ---------------------------- | ----------- | ------------- | ----- | --------- | ----- |
| `qwen3-coder:30b`            | 3.08        | 48459         | 3850  | 20/25     | 18/25 |
| `qwen3.6:27b-coding-nvfp4`   | 3.07        | 48535         | 42538 | 20/25     | 21/25 |
| `qwen2.5-coder:7b`           | 3.08        | 24721         | 3850  | 19/25     | 22/25 |
| `qwen3.5:9b`                 | 3.07        | 48537         | 3965  | 16/25     | 18/25 |
| `qwen3.8:27b-nvfp4`          | 3.07        | 48539         | 42542 | 16/25     | 17/25 |
| `nemotron-3.5-lightning:30b` | 3.07        | 48600         | 4027  | **13/25** | 20/25 |
| `nemotron-3-nano:30b`        | 3.07        | 48611         | 4027  | **12/25** | 17/25 |
| `nemotron-3-nano:4b`         | 2.81        | 48559         | 4374  | **6/25**  | 8/25  |

**The two `nvfp4` builds are an exception, and they show that the allocation is not an enforced cap.** They never dropped the filler. Under the fixed-filler sweep, archived in [`m1max-64gb-ollama0333-fill28000/`](tool/model_probes/context_fill_results/m1max-64gb-ollama0333-fill28000), they evaluated 42542 and 42538 tokens against the 35851 that `/api/ps` reported allocating, roughly 6,700 tokens past it, and scored 17/25 and 21/25 rather than the collapse an overflow would predict. Every other model measured loses the whole message at that boundary; these two crossed it at no cost. So `min(requested, trained window)` describes what the runner allocates, not what it enforces, and on these builds the two come apart. An intermediate run at the same 42.5k prompt inside a 65536 window returned figures identical to the sweep's, which is corroboration rather than evidence, and it was superseded by the calibrated run in the table above. No mechanism is established. Both are `nvfp4` builds, which this file already records as [flipping their `format` verdict](#not-a-card-test-the-format-canary) between runtimes, so a runner-specific difference is consistent with the readings and is not confirmed from server logs.

`qwen2.5-coder:7b` is the row a larger ceiling could not fix. Its trained window is 32768, and under `min(requested, trained window)` a bigger `--num-ctx` changes nothing, so it takes a smaller filler instead: 20000 tokens rather than 42000, hence its lower prompt count. At 24721 tokens it still runs its allocated window about three quarters full, which is as full as a 32768-capped model can be driven while leaving the reply somewhere to go.

**Calibration closes most of the gap between the flag and the measurement, and does not close all of it.** These runs asked for 42000 tokens of filler and, after subtracting each model's own system-prompt cost, delivered roughly 43.2k to 43.6k: about **3% over**. `qwen2.5-coder:7b` at a 20000 target landed within 0.6%. The residue has a plain cause rather than being noise. The filler is indexed (`filler-term4012 means concept806.`), so its digit strings lengthen as it grows, and a 118,000-character filler tokenizes slightly denser than the 10,000-character sample calibration measures. The direction is therefore size-dependent, and an earlier undershoot recorded at a much smaller fill does not generalise. Read **Prompt tokens** as the measurement in every case; every run here still fits its window, which is what the flag exists to protect.

**The fit control reproduced on the 16 GB host to the token and to the case, before either side was calibrated.** The three models a 16 GB machine can hold under these windows were re-run there 2026-09-11, in [`m5-16gb-ollama0333-fitcontrol-uncalibrated/`](tool/model_probes/context_fill_results/m5-16gb-ollama0333-fitcontrol-uncalibrated). Every prompt count and every pass count is identical to the M1 Max reading, including `nemotron-3-nano:4b` carrying 46287 tokens inside a 65536 window on a host with a quarter the RAM. At `temperature 0` an identical pass count across two hosts is close to what a correct measurement should produce rather than a second opinion, but it does establish that neither ingestion nor coverage under a filled window depends on host memory here.

| Model                | Fill / `num_ctx` | M5 tokens | M1 Max tokens | M5 pass | M1 Max pass |
| -------------------- | ---------------- | --------- | ------------- | ------- | ----------- |
| `nemotron-3-nano:4b` | 28000 / 65536    | 46287     | 46287         | 6/25    | 6/25        |
| `qwen3.5:9b`         | 28000 / 65536    | 42540     | 42540         | 19/25   | 19/25       |
| `qwen2.5-coder:7b`   | 12000 / 32768    | 19974     | 19974         | 21/25   | 21/25       |

**The M1 Max column above is a superseded run, cited rather than archived.** Both sides of this comparison were taken with the fixed filler. `m5-16gb-ollama0333-fitcontrol-uncalibrated/` still holds the M5 side; the M1 Max side was overwritten by the calibrated re-measurement in the table further up and survives in commit `9698553`. The comparison is reported as it was measured, since a cross-host reading taken under one condition on both hosts is what it establishes, and re-running only one side would not preserve it. What carries forward independently is that the Nemotron counts are unchanged by calibration: `nemotron-3-nano:4b` reads 6/25 on both hosts uncalibrated at 46287 tokens, and 6/25 again on the M1 Max calibrated at 48559.

**Re-run calibrated, the cross-host comparison holds and both sides are now archived.** The three models were measured again on the M5 on 2026-09-12 under the same calibrated parameters the M1 Max used, in [`m5-16gb-ollama0333-fitcontrol-calibrated/`](tool/model_probes/context_fill_results/m5-16gb-ollama0333-fitcontrol-calibrated), so the reading above no longer has to rest on a superseded commit. Prompt counts and allocations match exactly, and the calibrated chars-per-token match **to every digit** — 2.808588268313219 for `nemotron-3-nano:4b`, 3.0734029484029484 for `qwen3.5:9b` — which is what calibration being a property of the tokenizer rather than of the host looks like when measured.

| Model                | Fill / `num_ctx` | M5 tokens | M1 Max tokens | M5 pass | M1 Max pass |
| -------------------- | ---------------- | --------- | ------------- | ------- | ----------- |
| `nemotron-3-nano:4b` | 42000 / 65536    | 48559     | 48559         | 6/25    | 6/25        |
| `qwen3.5:9b`         | 42000 / 65536    | 48537     | 48537         | 16/25   | 16/25       |
| `qwen2.5-coder:7b`   | 20000 / 32768    | 24721     | 24721         | 20/25   | 19/25       |

One case separates the hosts on `qwen2.5-coder:7b` and nothing separates them on the other two, across nine content columns.

**Do not read the medians in this section as a host comparison. One model moved 3.8x against itself on one machine.** `nemotron-3-nano:4b` on the M5 medians **13356 ms** in the uncalibrated fit control of 2026-09-11 at 46287 tokens, and **3506 ms** in the calibrated run of 2026-09-12 at 48559 tokens — the same host, the same model, a 5% larger prompt, a day apart. Nothing about the host or the prompt accounts for that, so machine state does. The cross-host direction flips with it: on 2026-09-11 the M5 read roughly 2x slower than the M1 Max on two of three models, and on 2026-09-12 it reads 2.09x and 1.66x **faster** on the two that carry the largest prompts. Occupancy does not order them either — halving the fill made `nemotron-3-nano:4b` slower (5201 ms at 25065 tokens against 3506 ms at 48559), the opposite of what a prefill-dominated call should do, while the other two moved the expected way. The pass and token columns reproduce across all of these runs; the medians do not, and no host or fill-level claim in this section rests on them.
...
**At a third fill level the cost does not scale with occupancy, and there is no threshold between 25k and 48.5k tokens.** The three M5-capable models were re-run 2026-09-12 with each window held at its calibrated value and the fill roughly halved, in [`m5-16gb-ollama0333-fitcontrol-calibrated-halffill/`](tool/model_probes/context_fill_results/m5-16gb-ollama0333-fitcontrol-calibrated-halffill), so occupancy is the only variable. The near-empty column is the fixed-filler sweep, where these models were sent a message they discarded and evaluated the system prompt and question alone; it is a different archive and an uncalibrated one, so it is a low-occupancy reading rather than a matched control.
...
Two models decline by one case per step and `qwen3.5:9b` does not decline at all — 17 to 14 to 16, a three-case spread wider than its endpoints differ, which is noise at `--samples 1`. No single model's steps clear that bar either. What the three do jointly is bound the shape of the effect: pooled coverage drops five cases between near-empty and half fill and **none at all** between half and full, so the earlier two-point reading should not be extrapolated as proportional to occupancy, and nothing here behaves like a cliff at a particular fill. Whether the five-case pooled drop is real needs `--samples 2` and more models, not a fourth fill level.
...
Two limits on the above. Every run in this section is `--samples 1`, against the `--samples 2` most of this file carries, so a one-case difference is noise and only the Nemotron drops of five to seven cases are large enough to read. And no pass column here is like-for-like with [the shape-coverage table](#shape-coverage--all-fifteen-models-as-shipped), which was measured on a different runtime with two prose turns as history rather than a filler block, so read these within this section rather than against that one.
...
Ollama 0.33.3 reports `prompt_eval_cached_count` on every reply — how many prompt tokens were served from the runner's prefix cache rather than re-evaluated. The server records it as `cachedPromptTokens` in `/status`. Produced by [`prefill_cache_probe.dart`](tool/model_probes/prefill_cache_probe.dart), a standalone diagnostic rather than one of the seven sweep probes: it scores no cards and `sweep.sh` does not run it. As of 2026-09-05 the probe supports `--json` and archives its run the same way the sweep probes do; it is deliberately not added to `expectedProbes` in `check_results.dart`, for the same reason it is not in `sweep.sh`. The M5 readings below predate `--json` and cannot be re-taken from this host — they remain a spot measurement transcribed from terminal output, not an archived row. Measured 2026-09-03 on the Apple M5 / 16 GB under Ollama 0.33.3, `llama3.2:latest`, one model resident, `t=0`, a ~2,100-token system prompt, `num_ctx` 8192:
