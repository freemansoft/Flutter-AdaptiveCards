...
| ------------------------------------------------- | ------- | ----- | -------------------- | -------------------------------------------------------- | ------- |
| gpt-oss:20b                                       | 12.8 GB | ❌    | candidate            | everyday 19/21 · stress 9/10 · **breaks on `format`**    | ✅      |
| granite4.1:3b                                     | 2.0 GB  | ✅    | candidate            | everyday 16/21 · stress 7/10 · stalls without the seed   | ⚠️      |
| granite4.1:8b                                     | 5.0 GB  | ✅    | launch set (16 GB)   | everyday 19/21 · stress 10/10 but 6 prose                | ✅      |
| hf.co/unsloth/Nemotron-3-Nano-30B-A3B-GGUF:latest | 22.9 GB | ❌    | candidate            | everyday 15/21 · stress 5/10                             | ✅      |
| llama3-chatqa:8b                                  | 4.3 GB  | ✅    | candidate            | everyday 21/21, stress 10/10 — **all prose, 0 cards**    | ❌      |
| llama3-groq-tool-use:8b                           | 4.3 GB  | ✅    | candidate            | everyday 18/21 · stress 7/10                             | ⚠️      |
| llama3.2:latest                                   | 1.9 GB  | ✅    | candidate            | everyday 19/21 · stress 7/10 — retired as default        | ⚠️      |
...
| qwen3.6:27b-coding-nvfp4                          | 18.4 GB | ❌    | candidate            | everyday 21/21 · stress 8/10 · ignores `format`          | ✅      |
| qwen3.8:27b-nvfp4                                 | 16.9 GB | ❌    | launch set (large)   | everyday 20/21 · stress 9/10 · ignores `format`          | ✅      |
...
**Four hours of sustained load costs the M5 1.20x.** `granite4.1:8b` ran first in the sweep from a machine that had been idle for hours, and a re-run taken 13 seconds after the sweep ended is 1.20x slower — uniform at every percentile (p25 1.15x, median 1.20x, p75 1.17x, max 1.23x), with zero stalls either way and byte-identical shape scores. Same silicon, same weights, same Ollama build, same prompt digests: the only variable is elapsed time under load.

**The mechanism is not established.** Thermal throttling is the obvious candidate — this is a fanless `Mac17,3`, and a proportional shift across the whole distribution is the shape a clock reduction would take, rather than the shape a few slow calls would take. But no die temperature or clock frequency was read: `powermetrics` needs privileges the probes do not take, and `pmset -g therm` reported no warning level throughout, which on Apple Silicon is weak evidence either way. Background OS activity, memory fragmentation, and accumulated runtime state are all time-varying and none was excluded. What is measured is a within-machine slowdown after sustained load; why it happens is inferred.

**Note what this figure cannot be.** Memory bandwidth and GPU core count are fixed properties of the machine; they cannot differ between two runs on the same box, so they are excluded here by construction rather than by evidence. They belong to the cross-host spread above, not to this one.

Because the first model was the only one measured on a rested machine, `granite4.1:8b`'s **hot** run is the one published, and every M5 row in the table is therefore a hot-machine figure. No correction factor is applied to any row: a measured bias is reportable, an estimated correction is not.
...
`qwen3.8:27b-nvfp4` returns the same 444-character card under `none`, `json`,
and `schema` alike. `gpt-oss:20b` returns a 401-character card under `none`, an
**empty body (0 chars)** under `json`, and 94 characters of prose under
`schema`.
...
A second `system` message placed after conversation history reliably reaches it — the one screening model where that delivery test was confirmed rather than inconclusive (see [Key findings](#key-findings)).
...
- **`--seed-card` on / off — no effect, and no harm.** Identical score either
  ...
  that position.
- **`--json-format json|schema` — inert.** Byte-identical 444-character card
  under `none`, `json`, and `schema`. It ignores `format` silently, exactly
  like `qwen3.6:27b-coding-nvfp4`; treat that as an `nvfp4` family trait to
  check rather than a one-model quirk.
