...
- **`qwen3.5:9b` — dropped, though the 2026-08-20 re-measurement narrows the gap.** It scores **19/25 with history against `qwen2.5-coder:7b`'s 18/25** — no longer the exact tie the original decision rested on. Against it: 6.1 GB versus 4.4 GB, **4.9 s/call versus 2.4 s** on the 0.33.2 latency table (6.9 against 2.3 on the removed 0.32.14 archive), and a thinking mode that has to be disabled to be usable at all. One shape does not outweigh twice the latency, so the decision stands, on cost now rather than on a tie. Full detail: [its per-model notes](#qwen359b).
...
3/3, and so does `granite4.1:3b` (17/25) on the runs where its calls
completed, the weakest model that produces cards at all. The sole exception never produced a first card to edit, so it
fails at turn 1 rather than at the cascade.

So **cascade ability is gated entirely on turn-1 card production**, which the
shape table already measures. Its `Cascade` column separates no two models that
produce a first card at all. The probe earns its place as a regression check — if a prompt
...
**Re-measured under Ollama 0.33.3, both `nvfp4` verdicts held** — `qwen3.6:27b-coding-nvfp4` on 2026-09-04 and `qwen3.8:27b-nvfp4` on 2026-09-07.\*\* `qwen3.8:27b-nvfp4` and `qwen3.6:27b-coding-nvfp4` both still read `honored` — see [`results-m1max-64gb-ollama0333/`](tool/model_probes/results-m1max-64gb-ollama0333). The other two verdicts were not re-run this pass. Ollama 0.33.3's changelog also adds "Honor GGUF model defined default parameters."
...
- **The M1 Max re-run owed by the `Input.Rating` palette edit landed 2026-09-07;
  ...
  live archive for that host at the time, so its one stale run —
...
Measured on two M-series Macs against a local Ollama, August-September 2026 — see [Performance, by host and runtime](#performance-by-host-and-runtime) for which figures came from which. Latency medians exclude the first call after a model load ([Performance, by host and runtime](#performance-by-host-and-runtime)); re-run before trusting a single reading.
