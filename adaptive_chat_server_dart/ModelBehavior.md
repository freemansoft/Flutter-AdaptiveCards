...
- **A full window costs three of eight models a quarter to a third of their shape coverage, in three different ways.** `nemotron-3.5-lightning:30b` stops producing cards (20/25 to 13/25, `prose` 1 to 10), `nemotron-3-nano:30b` keeps producing them and substitutes a `TextBlock` for the input a case asked for (17/25 to 12/25, `no-input` 4 to 8), and `qwen3.8:27b-nvfp4` stops finishing its replies (21/25 to 16/25, `broken` 0 to 7: four 180 s stalls and three bodies that do not parse). A check that a reply parsed as a card catches the first and the malformed bodies; the stalls need a timeout, and the `TextBlock` substitution passes every structural check. The other five models in the table move by three cases or fewer, so an empty-window score does not predict which way a model moves. See [a filled context](#a-filled-context-an-oversized-history-message-is-dropped-whole-and-a-real-one-costs-some-models-a-third-of-their-shapes).
- **A history message that exceeds the allocated window is dropped whole, not trimmed, and nothing says so.** Ollama allocates `min(requested, trained window)`, confirmed on both a 16 GB and a 64 GB host with no counterexample; a message that does not fit inside that allocation is removed in full rather than cut down, and five of eight models measured under one such overflow evaluated only the system prompt and question, between 3819 and 4374 tokens. Sizing history in characters is what causes an overflow to go undetected: the same text tokenizes at 4.30 chars/token on llama, granite and gpt-oss builds and at 2.74 to 2.99 on Qwen and Nemotron ones, so a filler sized to fit one model's tokenizer overflows another's allocation by nearly half. Two `nvfp4` builds are an exception and evaluate thousands of tokens past their own allocation, scoring within one case of a full window that fits, so the allocation is enforced inconsistently rather than never. The fixed-filler sweep reproduced to the token and to the case under Ollama 0.34.0 on all fourteen models. The reply reads as a normal answer to a question asked with no history, so the loss is invisible unless `prompt_eval_count` is checked. See [a filled context](#a-filled-context-an-oversized-history-message-is-dropped-whole-and-a-real-one-costs-some-models-a-third-of-their-shapes).
...
`granite4.1:3b`'s row is cascade-damaged and is not a model measurement: its after-eviction unaided run stalls on calls 0-20, the probe's cold opening cases, and again on 89-99 — the queue-cascade signature described in [the cascade section](#stalls-are-a-queueing-cascade-not-a-runtime-difference) — so its 17/25, 12/25, and 9/25 record queued calls scored as failures rather than the model's coverage.
...
**The M1 Max column is a mixed harness.** [`results-m1max-64gb-ollama0332/`](tool/model_probes/results-m1max-64gb-ollama0332) holds 13 models measured **before runner eviction** (the 2026-09-01 sweep) and 2 measured **after runner eviction** (`llama3.2:latest` and `granite4.1:3b`, re-run 2026-09-02); the harness change is described in [the cascade section](#stalls-are-a-queueing-cascade-not-a-runtime-difference). Eviction is a no-op unless a call times out; 11 of the 13 recorded zero stalls, and the other two (`hf.co/unsloth/…` at 3, `nemotron-3-nano:4b` at 1) sit within one stall of their 0.32.14 counts (2 and 1 in the archive as it stood when it was removed), so the split does not touch their rows. `granite4.1:3b`'s sweep and stall cells (124.0 min, 52) are its after-eviction run, whose stall positions still carry the cascade signature; they are not model figures, and only its median should be read.
...
**On the archived 0.32.14 measurement, `granite4.1:3b` was not the worst staller — the M5 was ahead there too.** It stalled once on the M5 against 13 on the M1 Max archive, with a 13-minute sweep against 34. That comparison predates the finding that 0.33.2 stall counts carry a queueing cascade (below); on 0.33.2 the M1 Max count for this model is 52, from a run whose stall positions still show the cascade signature, so it is not a count of independently slow calls and should not be read against the M5's single stall the way the archive counts were.
...
**Stalls, not token rate, decide how long a sweep takes.** On the 0.33.2 M1 Max column, `granite4.1:3b` is the second-smallest model here and its sweep runs over two hours against `qwen3-coder:30b`'s ten minutes at eight times the weight, because 52 of its calls read as stalled. That count is consistent with a queueing cascade rather than 52 independently slow calls — see [the cascade section below](#stalls-are-a-queueing-cascade-not-a-runtime-difference). Budget a sweep by stall risk regardless of mechanism: one runaway generation can cost hours, and gigabytes tell you nothing about which model will produce one.
...
| `granite4.1:3b`   | 33.9 min / 13 stalls | 123.5 min / 52 stalls                   | 79.9 min / 36 stalls                       | 124.0 min / 52 stalls, cascade-damaged |
...
**What the section establishes**, measured across fifteen models on the M1 Max under Ollama 0.34.0 and set out in the two subsections that follow. An exact repeat of the runner's last call, and a growing conversation that extends it, reuse the cache on every model measured. A new conversation on a system prompt the runner already holds does not, and what it costs follows the model's memory type as `llama-server` reports it at load, then the runner:

- attention-only and sliding-window models reuse the prompt;
- a recurrent model on `llama-server` re-processes the tokens after its last context checkpoint, a fixed 1,024-token batch, on every new conversation and on every turn of two interleaved ones;
...
The subsection above left runner, quantization and architecture confounded. Three installed models separate architecture from the other two, all on `llama-server`, all run through the seven-phase probe once on 2026-09-22 (M1 Max / 64 GB, Ollama 0.34.0, `t=0`, `num_ctx` 8192, 300 entries): `qwen3.5:9b` (`qwen35`, the Qwen3.5 family in GGUF, `Q4_K_M`), `nemotron-3-nano:4b` (`nemotron_h`, `Q4_K_M`) and `gpt-oss:20b` (`gptoss`, `MXFP4`). Archives: `results-m1max-64gb-ollama0340/{qwen3.5_9b,nemotron-3-nano_4b,gpt-oss_20b}/prefill_cache_probe-entries300-sevenphase.json`.
...
| -------------------- | -------------- | ------------------ | ------------ | ------------------------------------ | ----------------------------------- | ------------------------ |
| `qwen3.5:9b`         | recurrent      | 3176/0, 8.7–11.0 s | 3176/3172    | 2152 cached on all 3, 3.4–4.0 s      | 2152–2153 on all 12, 3.2–3.8 s      | 3176/2152, 3.40 s        |
| `nemotron-3-nano:4b` | recurrent      | 3478/0, 5.4–6.4 s  | 3478/3474    | 2454 cached on all 3, 1.99–2.04 s    | 2454–2455 on all 12, 1.99–2.05 s    | 3478/2454, 1.96 s        |
| `gpt-oss:20b`        | sliding window | 2192/0, 2.4–3.1 s  | 2192/2191    | 2187 cached on all 3, 60–73 ms       | 2187 on 11 of 12; 1167 once, 1.56 s | 2192/1167, 1.51 s        |

**Both recurrent models re-process the last 1,025 tokens of the shared prefix on every new conversation, the first included, and never fall to a full cold prefill.** `llama-server` logs why. Processing the 3,176-token prompt it writes two context checkpoints, `created context checkpoint … n_tokens = 2152` and `… n_tokens = 3172`; Ollama starts it with `-b 1024 -ub 1024`, and the first sits one batch before the end. A new conversation then logs the fallback: `checking checkpoint with [3171, 3171] against 3167`, `checking checkpoint with [2151, 2151] against 3167`, `restored context checkpoint (… n_tokens = 2152 …)` on `qwen3.5:9b`, and `n_tokens = 2454` on `nemotron-3-nano:4b`. The `forcing full prompt re-processing due to lack of cache data (likely due to SWA or hybrid/recurrent memory …)` line belongs to requests with nothing to restore (a first request, an unrelated glossary, the aborted retry call), not to a new conversation. A new conversation diverges about 10 tokens from the end, past the last checkpoint that still matches, so the runner resumes from the earlier one. The cost is about a third of a cold prefill, every time: 3.2 to 4.0 s against 8.7 to 11.0 s on `qwen3.5:9b`, about 2.0 s against 5.4 to 6.4 s on `nemotron-3-nano:4b`. An exact repeat and a growing conversation's turns 2 and 3 restore the end checkpoint and stay warm. The unrelated request reuses nothing, not even the instruction prefix (`cached=0`), since no checkpoint sits there.

**`gpt-oss:20b`'s sliding-window layers go through the same checkpoint logic, with a milder result.** New conversations were warm on 14 of 15, and twice the runner restored a checkpoint at 1,167 tokens instead (after the unrelated request, and once after an exact repeat). An `MXFP4` 4-bit float quantization on `llama-server` therefore does not by itself produce the miss.

**This reframes the MLX result as the same architecture under a different checkpoint policy.** `qwen3_5` (the safetensors name) and `qwen35` (the GGUF name) are, on the evidence of the family and the naming, the same Qwen3.5 architecture; `llama-server`'s log confirms recurrent memory only for the GGUF build, so the MLX builds' recurrence is inferred, not logged. On that reading the three behaviors line up by memory type and runner:

| Memory type          | Runner         | A new conversation on a cached system prompt                                                    |
| -------------------- | -------------- | ----------------------------------------------------------------------------------------------- |
| standard attention   | `llama-server` | warm, every time (`llama3.2:latest`, `qwen3-coder:30b`)                                         |
| sliding window       | `llama-server` | warm on 14 of 15; a checkpoint rollback otherwise (`gpt-oss:20b`)                               |
| recurrent            | `llama-server` | re-processes the last ~1,025 tokens, every time (`qwen3.5:9b`, `nemotron-3-nano:4b`)            |
| recurrent (inferred) | MLX            | a full cold prefill the first time, then warm (`qwen3.8:27b-nvfp4`, `qwen3.6:27b-coding-nvfp4`) |

The MLX runner's `prefix_cache` log lines (`matched=3166 cached=4` on the first new conversation, `matched=3166 cached=3166` after) are consistent with it saving a restorable state only at the end of a processed prompt and adding one at a divergence point once a request has diverged there. That policy is inferred from the log, not read from source. `nvfp4` remains unseparated from the MLX runner, since every MLX-served model measured is `nvfp4`; a checkpoint policy is a runner property, so the quantization is not the likely cause, but no run has shown it. A standard-attention model served on MLX would separate the runner's policy from the architecture; none is installed.

**The long retry is a timing effect, not an MLX one.** `qwen3.5:9b` retried with 3473 of 3477 cached and 103 ms of prefill inside a 10.5 s call, about its cold request (10.9 to 13.2 s total). Its cold prefill, like the MLX builds', outlasts the probe's 0.4 s abort plus 5 s wait. `nemotron-3-nano:4b` (cold prefill about 5.4 to 6.4 s) retried in 4.2 s, and `gpt-oss:20b` (about 2.4 to 3.1 s) in 1.0 s. The retry's wall clock tracks how much of the abandoned prefill the wait did not cover, across both runners.

**The remaining eight installed models extend both groups (2026-09-22, same host, runtime and probe, one seven-phase run each).** Server-log memory type first, then prompt / cached per call:

| Model                                               | Memory (log) | Cold first request, prefill | First new conversation, 3 glossaries | Later new conversations, 12                           |
| --------------------------------------------------- | ------------ | --------------------------- | ------------------------------------ | ----------------------------------------------------- |
| `qwen2.5-coder:7b`                                  | standard     | 3174/5–16, 7.3–9.3 s        | 3167 on all 3, 101–134 ms            | 3167–3168 on all 12, 103–134 ms                       |
| `llama3-chatqa:8b`                                  | standard     | 2127/5–16, 4.1–5.9 s        | 2121 on all 3, 98–139 ms             | 2121 on all 12, 75–103 ms                             |
| `llama3-groq-tool-use:8b`                           | standard     | 2133/7–18, 4.1–5.9 s        | 2126 on all 3, 90–123 ms             | 2126 on all 12, 94–134 ms                             |
| `granite4.1:3b`                                     | standard     | 2131/5–16, 2.2–3.2 s        | 2124 on all 3, 49–55 ms              | 2124 on all 12, 46–75 ms                              |
| `granite4.1:8b`                                     | standard     | 2131/5–16, 5.7–7.8 s        | 2124 on all 3, 123–175 ms            | 2124 on all 12, 118–174 ms                            |
| `nemotron-3-nano:30b`                               | recurrent    | 3177/0, 4.0–5.3 s           | 2153 on all 3, 1.55–1.84 s           | 2153–2154 on all 12, 1.68–1.86 s                      |
| `nemotron-3.5-lightning:30b`                        | recurrent    | 3177/0, 4.2–5.2 s           | 2153 on all 3, 1.73–1.74 s           | 2153–2154 on all 12, 1.72–1.80 s                      |
| `hf.co/unsloth/Nemotron-3-Nano-30B-A3B-GGUF:latest` | recurrent    | 3178/0, 4.1–5.0 s           | 2154 on 2 of 3; 3162 once (172 ms)   | 3162 on 9 of 12 (168–184 ms); 2154 on 3 (1.76–1.78 s) |

`granite4.1` is standard attention by the log (`n_swa = 0`, no recurrent memory). Every standard-attention model restored the shared system prompt on every new conversation, the first included. Every Ollama-library recurrent build rolled back to the checkpoint 1,024 tokens before the prompt's end (2,153 of 3,177) on every new conversation. The unsloth build of the same Nemotron weights did so only when the call before was an exact repeat or a growing-conversation turn. Its own prefill writes a third checkpoint 16 tokens from the end (`n_tokens = 3162`), which those two call types do not leave in place; after any other call the checkpoint survives and the next new conversation restores from it, about 0.17 s against 1.75 s. Why `llama-server` places a checkpoint there for this build and not for the Ollama-library ones is not in the log. Its prompt is one token longer (3,178) under its own chat template; why that changes which checkpoint survives is not read from the log here.

**Every 0.34.0 seven-phase run, one row per model.** Prompt / cached tokens, so that each article figure traces to a row here. `Exact repeat` is the immediate repeat in phase 1; `turns 2-3` gives the tokens each growing-conversation turn re-evaluated; `after unrelated` is phase 4's `back-to-a`; `retry` gives prefill and total. Memory type is what `llama-server` printed at load (`llama_memory_recurrent`, `n_swa`); the two MLX rows are inferred from the model family.

| Model                                               | Memory         | Runner         | Exact repeat | turns 2-3 | After unrelated   | Retry                     |
| --------------------------------------------------- | -------------- | -------------- | ------------ | --------- | ----------------- | ------------------------- |
| `llama3.2:latest`                                   | attention      | `llama-server` | 2143/2142    | 16, 15    | 2143/2136, 43 ms  | 2444/2443, 36/157 ms      |
| `qwen3-coder:30b`                                   | attention      | `llama-server` | 3174/3173    | 17, 16    | 3174/3167, 72 ms  | 3475/3474, 22 ms/2.78 s   |
| `qwen2.5-coder:7b`                                  | attention      | `llama-server` | 3174/3173    | 17, 16    | 3174/3167, 89 ms  | 3475/3474, 31 ms/4.95 s   |
| `llama3-chatqa:8b`                                  | attention      | `llama-server` | 2127/2126    | 13, 12    | 2127/2121, 78 ms  | 2428/2427, 24 ms/1.60 s   |
| `llama3-groq-tool-use:8b`                           | attention      | `llama-server` | 2133/2132    | 16, 15    | 2133/2126, 111 ms | 2434/2433, 24 ms/1.79 s   |
| `granite4.1:3b`                                     | attention      | `llama-server` | 2131/2130    | 16, 15    | 2131/2124, 58 ms  | 2432/2431, 28/177 ms      |
| `granite4.1:8b`                                     | attention      | `llama-server` | 2131/2130    | 16, 15    | 2131/2124, 109 ms | 2432/2431, 35 ms/4.10 s   |
| `gpt-oss:20b`                                       | sliding window | `llama-server` | 2192/2191    | 11, 10    | 2192/1167, 1.51 s | 2493/2492, 19 ms/1.04 s   |
| `qwen3.5:9b`                                        | recurrent      | `llama-server` | 3176/3172    | 21, 20    | 3176/2152, 3.40 s | 3477/3473, 103 ms/10.49 s |
| `nemotron-3-nano:4b`                                | recurrent      | `llama-server` | 3478/3474    | 37, 37    | 3478/2454, 1.96 s | 3779/3775, 61 ms/4.18 s   |
| `nemotron-3-nano:30b`                               | recurrent      | `llama-server` | 3177/3173    | 28, 28    | 3177/2153, 1.63 s | 3478/3474, 68 ms/2.17 s   |
| `nemotron-3.5-lightning:30b`                        | recurrent      | `llama-server` | 3177/3173    | 24, 23    | 3177/2153, 1.69 s | 3478/3474, 57 ms/2.34 s   |
| `hf.co/unsloth/Nemotron-3-Nano-30B-A3B-GGUF:latest` | recurrent      | `llama-server` | 3178/3174    | 33, 33    | 3178/2154, 1.70 s | 3479/3475, 59 ms/1.36 s   |
| `qwen3.8:27b-nvfp4`                                 | recurrent?     | MLX            | 3176/3171    | 35, 35    | 3176/3171, 167 ms | 3477/2063, 17.6/39.3 s    |
| `qwen3.6:27b-coding-nvfp4`                          | recurrent?     | MLX            | 3176/3172    | 21, 20    | 3176/3172, 140 ms | 3477/3473, 142 ms/43.9 s  |

An immediate exact repeat leaves 1 to 5 tokens uncached on every model, and a growing-conversation turn re-evaluates 10 to 37. A repeat that follows the unrelated glossary is a different matter: it stays warm on the seven attention-only builds and on both MLX builds, and costs the five recurrent `llama-server` builds their new-conversation price again. `gpt-oss:20b` falls back to a checkpoint at 1,167 tokens there, which it otherwise reaches on only one of its fifteen new conversations. The per-model cold, first-new-conversation and later-new-conversation figures are in the two tables above.

Across all fifteen measured models the new-conversation behavior sorts by memory type first and runner second:
...
| sliding window       | `llama-server` | 1: `gpt-oss:20b`                                                                                                                             | warm on 14 of 15; a rollback to 1,167 once                                                                       |
...
| recurrent (inferred) | MLX            | 2: `qwen3.8:27b-nvfp4`, `qwen3.6:27b-coding-nvfp4`                                                                                           | a full cold prefill the first time, then warm                                                                    |
...
`--system-file` sends a real system prompt in place of the generated glossary, appending a session tag line per prompt so the probe still has five that differ. Run with the chat server's own [`assets/card_system_prompt.txt`](assets/card_system_prompt.txt), 15,124 characters over 223 lines of prose, rules and a JSON schema, against four models on 2026-09-23: `llama3.2:latest` (attention), `qwen3.5:9b` and `nemotron-3-nano:4b` (recurrent on `llama-server`), and `qwen3.8:27b-nvfp4` (recurrent on the MLX runner). Same host, runtime and `num_ctx` 8192 as the glossary runs, so the prompt is the only thing that changes. Archives: `prefill_cache_probe-cardprompt-ninephase.json` per model.

| Model                | Prompt, glossary → card | New conversation, glossary | New conversation, card prompt |
| -------------------- | ----------------------- | -------------------------- | ----------------------------- |
| `llama3.2:latest`    | 2144 → 3832             | 8 tokens re-evaluated      | 8 tokens                      |
| `qwen3.5:9b`         | 3177 → 3961             | 1025                       | 1025                          |
| `nemotron-3-nano:4b` | 3479 → 4371             | 1025                       | 1025                          |
| `qwen3.8:27b-nvfp4`  | 3177 → 3959             | 3173, then warm            | 3957, then warm               |

Nothing moves. The rollback is 1,025 tokens on a prompt 784 tokens longer whose content is nothing like 300 uniform clauses, which is what a positional checkpoint predicts and a content-sensitive one does not. Exact repeats leave 1 to 5 tokens uncached as before, and interleaved conversations still pay 1,024 to 1,046 per turn on the recurrent `llama-server` builds against 11 to 27 on the MLX build.

The first-divergence phase reproduces too. A second card prompt, unseen in the run, prefills cold (0 cached on `qwen3.5:9b`, 5 on `qwen3.8:27b-nvfp4`, 18 on `llama3.2:latest`, against prompts of 3,832 to 4,371 tokens). Its first new conversation then costs `qwen3.8:27b-nvfp4` a second cold prefill, 5 cached of 3,958, and its second is warm at 3,948. `qwen3.5:9b` and `nemotron-3-nano:4b` fall back one batch on both, and `llama3.2:latest` is warm on both.

**A first version of `--system-file` appended the tag and voided the first-divergence phase.** Five prompts that share every token but the last few are not five prompts for cache purposes: `delta-cold`, a prompt "no earlier call sent", reused 3,938 of 3,958 tokens on `qwen3.8:27b-nvfp4` and 2,937 on `qwen3.5:9b`, which is the alpha prompt's own checkpoint. Phases 1 to 4, 8 and 9 were unaffected, since they compare a prompt against itself. The tag now leads, so the prompts diverge at their first line the way the glossaries do. The appended-tag runs are kept as `-cardprompt-tailtag-` archives: they are the measurement showing that two prompts sharing a 3,940-token prefix reuse it, on both runners.
...
Three readings hold on both the second host and the second model, now confirmed by a third archived run apiece:

- **Identical repeat.** Cold prefill in the seconds against a warm repeat in tens of milliseconds, on both models, on every run measured (`llama3.2`: 2079 ms → 12 ms on 2026-09-04, 1900 ms → 11 ms archived; `qwen3.8:27b-nvfp4`: 37779 ms → 133 ms, confirmed at 158 ms on the 2026-09-04 repeat run and at 183 ms on the 2026-09-05 archived run).
- **A growing conversation pays for its new tokens only, on both models.** `qwen3.8:27b-nvfp4`'s per-turn prefill (812 ms, 804 ms on 2026-09-04; 430 ms, 802 ms, 812 ms on that day's repeat run; 405 ms, 768 ms, 770 ms for turns 1–3 on the 2026-09-05 archived run) runs higher than `llama3.2`'s (54 ms, 53 ms; 39 ms, 54 ms, 53 ms archived) but stays an order of magnitude below its own cold prefill on every run, consistent with the same behavior at a higher per-token cost.
- **The cache survives interleaving, on both models — and the unrelated request's own near-total miss is not a `qwen3.8`-specific finding.** Comparing each model's `other` reading against its own cold prefill from the same run: `llama3.2` read 2444/28 cached at 2170 ms archived, against a 1900 ms cold prefill the same run — 1.14x. `qwen3.8:27b-nvfp4` read 3176/4 cached at 37815 ms archived, against a 34277 ms cold prefill the same run — 1.10x, and the same ratio holds on the other three `qwen3.8` runs (40381/37779 = 1.07x on 2026-09-04, 40604/37445 = 1.08x on that day's repeat run using its own recorded cold prefill, 36601/35025 = 1.04x on the unarchived corroboration run). Every one of five readings across two models lands within 15% of that run's own cold-prefill cost. The unrelated (`bravo`) prompt shares only a fixed ~28-token instruction prefix with the cached (`alpha`) sequence before diverging into a different glossary entirely (`_systemPrompt()` in the probe), so near-total re-evaluation is the expected result of sending a genuinely different prompt, on any model — not a property distinguishing `qwen3.8` from `llama3.2`. What both models do keep is the original sequence's cache slot: the repeat immediately after the unrelated request comes back warm, every run, on both models — `llama3.2` 2143/2136 at 39 ms (2026-09-04 and 2026-09-05 archived, an exact match); `qwen3.8:27b-nvfp4` 3176/3171 at 215 ms and 205 ms on 2026-09-04, 198 ms on 2026-09-05 archived (165 ms on the unarchived corroboration) — all in the same range as each model's own identical-repeat warm-repeat figure. This is the same pattern the M5 measurement's "cache survives interleaving" reading describes, and the M5 table's row for this step (2143/2136, 55 ms) is a like-for-like match to the repeat step here, not to the unrelated request — the M5 measurement was never mismatched the way the M1 Max table briefly was.

One reading is a confirmed miss on the large model, reproduced across four measurements (three independent runs, one unarchived corroboration):

- **"Same system prompt, different question" reproduces the M5 pattern for `llama3.2` on this host, on every run, but not for `qwen3.8:27b-nvfp4`, on any run.** For `llama3.2`, this step reused nearly the full system prompt (87 ms on 2026-09-04, 42 ms on the 2026-09-05 archived run, ~2,136 of ~2,144 tokens cached both times). For `qwen3.8:27b-nvfp4`, the same step came back with `cached=4` of ~3,177 tokens and a prefill matching its own cold prefill on the two 2026-09-04 runs (40426 ms, 40377 ms) and again on the 2026-09-05 archived run (37615 ms) — three independent runs, all misses, within 8% of each other and nowhere near the 133–183 ms warm repeat measured for identical-repeat. The same unarchived same-day rerun mentioned above also repeated this step and read 36685 ms, `cached=4`, reported here as corroboration and not as a counted fourth point, since it was not archived. A byte-identical system prompt with only the trailing question differing does not reuse the cache on this model, on this host, across three measurements — unlike the interleaved-unrelated-request reading above, this one has no plain structural explanation: the divergent tail here is one short question, not an entirely different ~2,000-token body.

One reading is unstable on the large model:

- **Retry after an abort costs a warm repeat on `llama3.2` on every run** (30 ms on 2026-09-04, 30 ms on the 2026-09-05 archived run, against 29 ms on the M5), **but the same step on `qwen3.8:27b-nvfp4` remains unstable.** The two 2026-09-04 runs read 148 ms with 3472 of 3477 cached, then 18154 ms with 2063 of 3477 cached. The 2026-09-05 archived run reads 142 ms with 3472 of 3477 cached — close to the first reading, not the second. The unarchived same-day rerun mentioned above also repeated this step and read 142 ms at 3472 cached as well, reported here as corroboration and not as a counted fourth point, since it was not archived. What has been observed is three counted readings — 148 ms, 18154 ms, 142 ms — plus one uncounted corroboration at 142 ms: three near 142–148 ms and one at 18154 ms; that is not a rate, and one anomaly among a handful of readings is a real observation about variability rather than noise to average away. Every reading is partial reuse rather than a full cold prefill (0 cached) or a full warm repeat (3477 cached), and the amount reused and the cost still differ by roughly two orders of magnitude between the fastest and slowest reading recorded. A plausible reason — how much of the aborted call's prefill had completed server-side before the 400 ms abort landed varies run to run, and the retry only needs to redo what was not yet computed — is consistent with partial-but-variable reuse; it is not confirmed, since the probe does not observe server-side progress directly.

For `qwen3.8:27b-nvfp4`, a byte-identical repeat and a strictly-growing conversation reuse the cache on every run measured; interleaving a genuinely unrelated request pays close to a full cold prefill, on every run measured, exactly as it does on `llama3.2`, without evicting the original sequence's cache slot, which stays warm afterward on every run measured; a fresh request sharing the system prompt without extending an already-cached sequence misses entirely on every run measured, unlike `llama3.2`; and a retry after an abort read close to a warm repeat on three of the four runs and far from it on one. Why the cross-question request misses entirely on `qwen3.8:27b-nvfp4` but not on `llama3.2`, and why the retry's reuse is variable, is not established — the probe reports `prompt_eval_cached_count`, not the runner's cache-key logic or its server-side scheduling — so a model- or quantization-specific matching rule, a context-length effect past ~3,000 tokens, and a slot-count limit are each consistent with the cross-question miss, and abort-timing variance is consistent with the retry instability, but none of these is confirmed.

- Caveat: single-turn-scale replies, and the runner's retention limits and eviction policy under memory pressure, remain unprobed. Identical-repeat, growing-conversation reuse, and interleaved-request cache survival reproduced across two hosts (M5, M1 Max) and two models (`llama3.2:latest`, `qwen3.8:27b-nvfp4`) under the same Ollama 0.33.3, confirmed by two further repeat runs for the large model (2026-09-04, 2026-09-05 archived) — that part of the shape is not an artifact of one machine, a 2 GB model, or a single measurement. Reusing a shared system prompt across a different fresh question is not general runtime behavior: it held for `llama3.2` on both hosts and missed entirely, across three independent runs plus one unarchived corroboration, for `qwen3.8:27b-nvfp4` on the M1 Max — the one reading in this section that is genuinely model-specific rather than a property of sending a long, mostly-different prompt. Retry-after-abort cost is not a fixed property of the runtime either: it matched the M5 figure for `llama3.2` on every run, but on `qwen3.8:27b-nvfp4` three of four readings clustered near 142–148 ms and one read 18154 ms. These figures are this runtime's behavior for two specific models on one host, not this runtime's cache behavior in general — and for the large model, one of the six readings is not stable across repeat runs of the same probe.

#### Ollama 0.34.0, the first four models (2026-09-21)

_Recorded as: only the first divergence from a cached prefix pays, on the MLX-served `nvfp4` `qwen3_5` builds. The first half holds. The attribution to the `nvfp4` quantization does not: an `int4` MLX build behaves the same way, as the batch section above records._
...
**The `ordering` phase (6) came back a near-total hit on both MLX builds, which turned out to test the wrong thing.** By the time it runs, phase 2's own cross-question call has already sent one fresh question against the shared glossary — so every later fresh question in phase 6, whatever call preceded it, is a _second or later_ divergence from that prompt, not a first one. On `qwen3.8:27b-nvfp4`: `seed` 3177/3166, `after-exact` 3177/3166, `after-question` 3177/3167, `after-question-2` 3177/3167, `after-exact-2` 3177/3167, `after-extend` 3177/3167 — every fresh question after the first one reused almost the whole prompt, regardless of whether an exact repeat, another fresh question, or a growing conversation immediately preceded it. `qwen3.6:27b-coding-nvfp4` matches, cell for cell. That result motivated phase 7: a system prompt no earlier call in the run had sent at all, so its first fresh question is a genuine first divergence.
...
On both MLX-served builds, `fresh-1` costs the same as `cold` — a full prefill, within a few percent, whether or not an exact repeat ran in between — and `fresh-2` and `fresh-3` both reuse almost the whole prompt. `llama3.2:latest` and `qwen3-coder:30b`, both on `llama-server`, show no such split: every fresh question reuses the prompt, `fresh-1` included, in both arms. The `qwen3-coder:30b` row comes from a seven-phase run added on 2026-09-22 ([`prefill_cache_probe-entries300-sevenphase.json`](tool/model_probes/results-m1max-64gb-ollama0340/qwen3-coder_30b/prefill_cache_probe-entries300-sevenphase.json)), same host and runtime.

**This resolves the cross-question miss as a property of the first divergence from a cached prefix on the MLX-served `nvfp4` `qwen3_5` builds, not of prompt length or of what request came before it.** On the MLX runner, only the first request that diverges from a prompt's cached prefill pays the cost of a cold one; every later divergence from that same branch, in any order tried, reuses it. The earlier section's "no plain structural explanation" no longer holds — there is one, bounded to this runner — but the mechanism inside the runner that ties a restore to having visited a branch once is not read from Ollama's source, only inferred from these readings, and stays a hedge. What it does explain: the five-phase probe's `new-question` step sends exactly one fresh question per system prompt, so on the MLX runner it was always this same first-divergence event — which is why it read as missing on every run of that step, on both `qwen3.8:27b-nvfp4` and `qwen3.6:27b-coding-nvfp4`. Whether the runner, the `nvfp4` format or the `qwen3_5` architecture is responsible is not separated by these four models.

**Retry-after-abort adds two corrections to the earlier reading.** Across this session's three `qwen3.8:27b-nvfp4` retry readings at 300 entries (five-, six-, and seven-phase archives) plus one at 200 entries, the retry read `cached=2063` on three of the four and `cached=3472` on the fourth — a repeat of the earlier section's instability, not a resolution of it. The recurring `2063` is the MLX runner's chunk boundary, and its log shows it: the aborted call logs `Prompt processing progress processed=2048`, then `Request terminated error="context canceled"`, and the retry logs `matched=2063 cached=2063`, the 2,048 processed tokens plus the 15 it already shared. The same three lines appear in the fivephase, sevenphase and 200-entry archives, which is why the figure does not move with prompt size. `qwen3.6:27b-coding-nvfp4`'s retry did not show this instability: `cached=3473` at 142 to 179 ms of prefill on all three readings this session, the model's retry behavior not measured before this session. `llama3.2:latest`'s retry read a full warm repeat on two of its three readings this session (`cached=2443`) and a partial one on the third (`cached=2076`, 796 ms) — the earlier section's "costs a warm repeat on `llama3.2` on every run" no longer holds without qualification; `llama-server`'s retry is close to deterministic here, not perfectly so. **`prompt_eval_duration` understates what the MLX retry cost.** The archives record `totalMs` for every call, and every retry on both MLX builds took 37.4 to 47.3 s of wall clock, about a full cold request, whether its prefill read 142 ms or 17.6 s; a warm identical repeat on the same builds totals 2.3 to 5.2 s. The 0.33.3 archive reads the same (142 ms prefill inside a 39,693 ms call), so the earlier section's 142–148 ms retries were fast prefills inside slow calls. The server log shows what the retry waits for: the abandoned call runs to completion, prefill and generation, before the retry is served. The aborted `qwen3.6:27b-coding-nvfp4` request returns `200 | 44.33 s`, and `qwen3.5:9b`'s aborted call finishes an 11.35 s prefill and returns at 13.5 s. The retry therefore pays whatever of the abandoned call the 0.4 s abort and 5 s wait left, which also explains `nemotron-3-nano:30b` waiting 2.2 s although its cold prefill is 4.0 to 5.3 s. `qwen3-coder:30b` also retried with a 20–22 ms prefill inside a 2.8–5.6 s call, against a ~0.3 s warm repeat. `llama3.2:latest`'s retries totaled 148–157 ms on its two warm readings.

**Identical-repeat, cold prefill, and the cross-question miss itself reproduce to the token across all three 0.34.0 archives per model**, confirming the 0.33.3 figures in the section above hold under the newer runtime: `qwen3.8:27b-nvfp4` reads cold 3176/0, identical 3176/3171, cross-question 3177/4 in every one of its three 300-entry archives (five-, six-, seven-phase); `llama3.2:latest` reads cold 2143/17, identical 2143/2142, cross-question 2144/2136 in every one of its three. Only the millisecond figures vary, run to run, as they do elsewhere in this notebook.

- Caveat: this closes the cross-question miss's cause as far as this probe can establish it — first-divergence-bound on the MLX-served `nvfp4` `qwen3_5` builds, and independent of prompt length and of the preceding request, with runner, quantization and architecture still confounded — but the runner's own logic for when it keeps a restorable branch point is not read from source, so "why the MLX runner does this" stays inferred rather than confirmed. Still unprobed, at the time this section was written: ten of the fourteen models in the roster (only `qwen3.6:27b-coding-nvfp4` and `qwen3-coder:30b` were added to this probe this session, alongside the two models it already covered), whether the pattern holds past two divergent branches per prompt, and the runner's retention limit and eviction policy under memory pressure.
