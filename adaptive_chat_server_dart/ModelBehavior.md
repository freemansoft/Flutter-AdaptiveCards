...
Two runs at `--fill-tokens 28000`, both under Ollama 0.33.3 at `--samples 1`, over the eight models a 16 GB host can hold. [`m5-16gb-ollama0333-fill28000/`](tool/model_probes/context_fill_results/m5-16gb-ollama0333-fill28000) was measured 2026-09-10 on the Apple M5 / 16 GB; [`m1max-64gb-ollama0333-fill28000/`](tool/model_probes/context_fill_results/m1max-64gb-ollama0333-fill28000) was measured 2026-09-11 on the Apple M1 Max / 64 GB. Both requested `num_ctx` 35851, which is the 28000-token fill target plus the card system prompt's own 3755-token estimate. **Prompt tokens** below is Ollama's `prompt_eval_count`, the floor across the 25 cases, and it is the whole measurement: a model that ingested the filler reports roughly 29.5k, and a model that did not reports the system prompt and question alone.

| Model                     | Trained window | Prompt tokens | Filler  | M5 pass | M1 Max pass |
| ------------------------- | -------------- | ------------- | ------- | ------- | ----------- |
| `llama3.2:latest`         | 131072         | 29546         | kept    | 8/25    | 9/25        |
| `granite4.1:8b`           | 131072         | 29536         | kept    | 8/25    | 8/25        |
| `granite4.1:3b`           | 131072         | 29536         | kept    | 3/25    | 3/25        |
| `qwen2.5-coder:7b`        | 32768          | 3850          | dropped | 22/25   | 22/25       |
| `qwen3.5:9b`              | 262144         | 3965          | dropped | 17/25   | 18/25       |
| `nemotron-3-nano:4b`      | 262144         | 4374          | dropped | 8/25    | 8/25        |
| `llama3-groq-tool-use:8b` | 8192           | 3825          | dropped | 10/25   | 10/25       |
| `llama3-chatqa:8b`        | 8192           | 3819          | dropped | 2/25    | 2/25        |
...
- **Why three models with room to spare still drop the filler, and what the runner actually allocated.** [The filled-context runs](#a-filled-context-five-of-eight-models-drop-the-filler-whole-and-two-hosts-agree) establish that the drop is whole-message, silent, and not caused by host memory. They do not establish the mechanism. `qwen3.5:9b` and `nemotron-3-nano:4b` carry 262144-token windows and `qwen2.5-coder:7b` carries 32768, all longer than the roughly 29.5k the prompt reaches, and all three discard it anyway. The probe records the `num_ctx` it requested rather than what the runner allocated, so the clamping seen on the M5 via `/api/ps` is neither confirmed nor excluded for these models; the one direct reading on the M1 Max, `granite4.1:8b` at `CONTEXT 35851`, covers a model that kept the filler. Capturing `/api/ps` per model inside the probe is the next step, and it is a probe change rather than another sweep.
