...
**Re-measured under Ollama 0.33.3 on 2026-09-04, both `nvfp4` verdicts held.** `qwen3.8:27b-nvfp4` and `qwen3.6:27b-coding-nvfp4` both still read `honored` — see [`results-m1max-64gb-ollama0333/`](tool/model_probes/results-m1max-64gb-ollama0333). The other two verdicts were not re-run this pass. Ollama 0.33.3's changelog also adds "Honor GGUF model defined default parameters"; the probes pin `temperature`, `think`, and `num_ctx` but not `top_p`/`top_k`/`repeat_penalty`, so figures sampled at `t=0.2`/`0.6` elsewhere in this file may not be comparable across the 0.33.2-to-0.33.3 upgrade, while `t=0` greedy figures are shielded from that change.
