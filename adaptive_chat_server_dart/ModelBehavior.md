...
**This is the current, as-shipped picture**, and the only model comparison in this file that is: all 25 cases, `t=0`, `--samples 2`, both conditions, with the unconditional card seed in place. It derives from the Ollama 0.33.2 runs in [`results-m1max-64gb-ollama033/`](tool/model_probes/results-m1max-64gb-ollama033) on the Apple M1 Max: thirteen models swept 2026-09-01 before runner eviction, and `llama3.2:latest` and `granite4.1:3b` re-run 2026-09-02 after runner eviction, otherwise under identical conditions. Sorted by with-history coverage, which is what a user actually experiences.
