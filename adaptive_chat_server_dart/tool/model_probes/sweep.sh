...
#   export SWEEP_RESULTS=tool/model_probes/results-m5-16gb-ollama0331
#   tool/model_probes/sweep.sh                # every model
#   tool/model_probes/sweep.sh granite4.1:8b  # just one
set -u

cd "$(dirname "$0")/../.." || exit 1
# Required, not defaulted. Results are per host -- results-m1max-64gb-ollama0332/,
# results-m5-16gb-ollama0331/ -- because latency is a property of the box as much as the
...
if [[ -z ${SWEEP_RESULTS:-} ]]; then
  ...
  echo "  SWEEP_RESULTS=tool/model_probes/results-m5-16gb-ollama0331 $0 $*"
  exit 2
fi
RES=$SWEEP_RESULTS
