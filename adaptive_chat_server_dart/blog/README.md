...
| 8   | On Ollama, a model's memory type and its runner decide how much of the cached prompt a new conversation reuses | `article-8-prompt-cache-*`        | [post](https://joe.blog.freemansoft.com/2026/09/on-ollama-models-memory-type-and-its.html)    | **Published** 2026-09-26 after a five-reviewer pass. Split from 5 on 2026-09-18. Fifteen models sorted by memory type and runner, the one-batch `llama-server` rollback measured across prompt sizes, two MLX controls, two interleaved conversations, the retry read from the archived server logs, and four checks for a chat server. Visuals: a flowchart and two sequence diagrams. Republished the same day with the `nemotron_h` MLX control, so the draft and the live post agree. About 2,260 prose words |
...
Article 8 was republished on 2026-09-26, the day it first went live. The
first version said nothing in its data tested the architecture behind the MLX
runner's first-divergence cost. That day a `nemotron_h` safetensors build,
`nemotron-3-nano-mlx:4b-bf16`, ran the nine-phase probe on the MLX runner and
reproduced the `qwen3_5` shape, so the cost is the runner's snapshot policy and
not the Qwen3.5 architecture; the same weights on `llama-server` show that
runner's rollback. The republish carries the control paragraph, a row in the
interleaved table, and the runner term, and drops the two failed attention-only
loads, which stay recorded in the notebook. No figure in the first version was
wrong; the republish resolves a gap that version had stated.
...
pay one extra full prefill per system prompt, then nothing, and two MLX
controls repeat the shape, an `int4` Qwen3.5 build and a `bfloat16`
`nemotron_h` build, which separates the runner's policy from both the
quantization and the architecture. The article gives `llama-server`'s log lines for
...
Its prose runs to about 2,260 words, past the 2,000 target, because the
