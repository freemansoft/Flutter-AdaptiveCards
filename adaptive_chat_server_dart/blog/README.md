...
| 8   | Recurrent-memory models on Ollama re-process a cached system prompt for new conversations | `article-8-prompt-cache-*`        | n/a                                                                                           | Not published. Split from 5 on 2026-09-18; review pass, restructured into four findings, two mermaid visuals 2026-09-20. Second review pass 2026-09-21: claim corrections, the retry confound named, restructured, retitled twice, and the bar chart cut as unreadable, leaving the sequence diagram as its visual. Third pass 2026-09-21: `prefill_cache_probe.dart` gained `--entries` and two ordering phases; the cross-question miss is now explained (runner-bound, first-divergence-bound) rather than left as an unresolved per-model cause, and the article was retitled and restructured around it |
...
Article 7 was 1290 and is 1838 after the
...
0.34.0. An exact repeat and a growing conversation reuse the cache on all
fifteen. **A new conversation's cost follows the model's memory type**, read
from `llama-server`'s load output: the eight attention-only models reuse the
cached system prompt; the five recurrent models on `llama-server` re-process
the tokens after the last context checkpoint, about 1,025 at this prompt size,
on every new conversation; the two `nvfp4` Qwen3.5 builds on the MLX runner pay
one extra full prefill per system prompt, then nothing. The article gives
`llama-server`'s log lines for the mechanism, marks the MLX runner's policy and
the MLX builds' recurrence as inferred, and names what the fifteen models do not
separate (the MLX runner from `nvfp4`). The retry after an abort, whose total
time tracks how much of the abandoned prefill the probe's fixed wait leaves
uncovered, on both runners.

Its prose runs to about 1,800 words. It carries a fifteen-row table because the
finding is a sort of fifteen models into three memory-and-runner groups, and a
smaller table would leave each group at one or two models. The 0.33.3 runs are
cited in one sentence, since their token counts repeat under 0.34.0.
...
M1 Max's calibrated parameters: prompt counts identical to the digit, two of
three scores unchanged, one case apart on the third. One case across three
models is the noise floor, so the check found no host effect to report.
Identical prompt counts are expected, since a tokenizer is a property of the
model and not of the host.

Article 7 carried that comparison as a three-row table until 2026-09-21, when
it was cut as a null result: the article is about what a full window costs, and
the table reported that a smaller host changes nothing. The figures stay
recorded here. Article 7 keeps its M1 Max attribution for the eight-row table,
since five of those rows are single-host either way.
