...
1. The M1 Max figures for `granite4.1:3b` and `llama3.2:latest` were measured
   **after runner eviction**, and the other six **before runner eviction**.
   Runner eviction is a harness change, described in the measurement-hygiene
   article in this series, that sends Ollama an unload after a call times out.
   It is a no-op unless a call times out, and those six recorded zero stalls on
   both hosts, so the comparison holds for them, checked rather than assumed.
