...
Two findings came out of filling it. Both were measured on an Apple M1 Max with 64 GB and an Apple M5 with 16 GB,
each running Ollama 0.33.3. The M1 Max results reproduced under Ollama 0.34.0. What the
...
| **`prompt_eval_count`** | The token count Ollama reports for the prompt it actually evaluated. Compared with the size of what was sent, it shows what the model really received, and this article turns on it.                                                                                                      |
...
card system prompt. It then reads Ollama's `/api/ps` to find out what the runner
actually gave it. Every row below requested 35851 tokens. That is 28000 for the filler, an
...
Thirty-one runs across both hosts produced no counterexample under Ollama
0.33.3. That includes six models too large for the 16 GB machine, measured on the M1
Max. They asked for 35851 against windows of 131072 and above and received
exactly that. The M1 Max then repeated its fourteen-model sweep under
Ollama 0.34.0, and every allocation, prompt token count and score came back the
same.
...
system prompt and the question and nothing else. Ollama removed the history
message in full.
...
than removing it, which the measurement-hygiene article in this series
records at 4,098 evaluated tokens against an 8,192-token window.
...
is actually allocated. All of them kept it. A later run under Ollama 0.34.0 did
the same for the two 8192-window models, with 2500 tokens of filler, and both
kept it too. Across ten models, none dropped a message it had room for.

The chat server in this repository now checks for overflow on every reply, and
it estimates the tokens it sent at four characters each. For this prompt that
estimate is about 31,750 tokens, under a 35851-token window, where
...
Neither build had an empty-window score. Later runs on the M1 Max under Ollama
0.34.0 supplied one, and a full window that fits for comparison. The
...
running past the allocation shows no cost of its own. Against the empty window, the filled runs of `qwen3.8:27b-nvfp4` score four
and five cases lower, and those of `qwen3.6:27b-coding-nvfp4` two and three.
These are single samples, and the notebook reads a difference only at five cases
or more. Only the five-case drop reaches that, on one run.
...
Four checks, in the order this article met them.
...
| Measure a model at the context length you will run it at | A full window costs some models about a third of their coverage, which the companion article decomposes.                   |
