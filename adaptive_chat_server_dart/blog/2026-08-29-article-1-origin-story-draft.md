...
the card body; the client renders it as a server-driven UI whose payload was generated by a language
...
what the sweeps turned up kept changing the server.
...
## Prose that renders is not the same as a card that answers
...
## Three checks inspect a reply, and only the client really parses it
...
decides card-vs-prose and extracts the body array it forwards to the client. The
probe scripts in
...
any depth. It warns and renders the card anyway, and no probe calls it.
...
Neither server-side check was designed in advance. Both grew out of the sweeps,
one observed reply at a time. The detector accepts three card shapes — a whole
`AdaptiveCard` object, a bare array of elements, a single element — because
...
| Shape (`shape_ab.dart`)              | **25** cases, scored cold-start and with history, `--samples 2` | Did the reply use the element type that answers this question?                                                    | Does the card do the job the question asked? |
...
- **Shape** — "what are my options for deployment targets", which passes only on
  an `Input.ChoiceSet`.
...
about them changing. (Shape figures are `--samples 2`; everyday and stress are
`--samples 1`.)
...
context. What that seed is worth varies by model, from +10 shapes to −2, and a
