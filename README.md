...
- Run the demo in **echo mode** (no LLM) to see the SDUI wiring on its own: the
...
- Point the server at a local **Ollama** chat model (`--ollama-url`) so replies are
  generated by an LLM instead of echoed, with conversation history threaded back
  into each prompt for context.
- Switch the server's **system prompt** to have the model reply with a live
  Adaptive Card fragment (dates, choice sets, FactSets, tables, ratings, and more)
  instead of Markdown text — the client renders whatever shape the server sends
  with no client code change, demonstrating dynamic, LLM-generated SDUI end to end.
