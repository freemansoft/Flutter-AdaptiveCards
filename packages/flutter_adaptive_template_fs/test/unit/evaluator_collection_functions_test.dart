...
void main() {
  group('collection functions', () {
    test('join concatenates with a separator', () {
      expect(
        evalExpression("join(items, ', ')", data: {
          'items': ['a', 'b', 'c'],
        }),
        'a, b, c',
      );
    ...
    test('first returns the first element', () {
      expect(evalExpression('first(items)', data: {'items': [10, 20]}), 10);
    });

    test('last returns the last element', () {
      expect(evalExpression('last(items)', data: {'items': [10, 20]}), 20);
    });

    test('sum adds numeric elements', () {
      expect(evalExpression('sum(items)', data: {'items': [1, 2, 3]}), 6);
    });

    test('average averages numeric elements', () {
      expect(evalExpression('average(items)', data: {'items': [2, 4]}), 3.0);
    });
  });
