...
| **`roundedCorners`**            | _([Teams extension](https://learn.microsoft.com/en-us/microsoftteams/platform/task-modules-and-cards/cards/cards-format))_ Radius resolved via HostConfig `cornerRadius` (default 8); wired on `Container`, `ColumnSet`, `Column`, `Table`; not yet exposed on `Image` | Low — opt-in JSON flag; `Image` wiring tracked separately                    |
