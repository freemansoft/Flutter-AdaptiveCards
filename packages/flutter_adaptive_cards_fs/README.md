...
| Media         | [spec](https://adaptivecards.io/explorer/Media.html)         | [learn](https://learn.microsoft.com/en-us/adaptive-cards/schema-explorer/media)           | ⚠️ Partial     | ⚠️ Limited | -                                                                                                                                                                                          | Video via `video_player`; poster attribute has issues; limited on desktop platforms                                                         |
...
| Icon          | [hub](https://adaptivecards.microsoft.com/)                  | —                                                                                         | ⚠️ Partial     | ✅ Yes     | [plan workstream C](https://github.com/freemansoft/Flutter-AdaptiveCards/blob/main/docs/superpowers/plans/2026-06-08-refresh-icon-charts-text-features.plan.md#workstream-c--icon-element) | ~68 Fluent names via Material icons; unknown → `help_outline`; `selectAction` supported                                                     |
...
| --------- | -------------------------------------------------------- | ------------------------------------------------------------------------------------ | -------------- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| Container | [spec](https://adaptivecards.io/explorer/Container.html) | [learn](https://learn.microsoft.com/en-us/adaptive-cards/schema-explorer/container)  | ⚠️ Partial     | ✅ Yes     | -                                                                                                                                                        | `minHeight` supported; `bleed` not implemented                                |
| ColumnSet | [spec](https://adaptivecards.io/explorer/ColumnSet.html) | [learn](https://learn.microsoft.com/en-us/adaptive-cards/schema-explorer/column-set) | ✅ Complete    | ✅ Yes     | [Column-ColumnSet-Fill-Vertical-Height.md](https://github.com/freemansoft/Flutter-AdaptiveCards/blob/main/docs/Column-ColumnSet-Fill-Vertical-Height.md) | Equal column heights fixed (`IntrinsicHeight` + stretch); see doc for history |
| Column    | [spec](https://adaptivecards.io/explorer/Column.html)    | [learn](https://learn.microsoft.com/en-us/adaptive-cards/schema-explorer/column)     | ✅ Complete    | ✅ Yes     | [Column-ColumnSet-Fill-Vertical-Height.md](https://github.com/freemansoft/Flutter-AdaptiveCards/blob/main/docs/Column-ColumnSet-Fill-Vertical-Height.md) | Same height fix as ColumnSet                                                  |
...
| `refresh`                      | spec (v1.4)                                                 | ✅ Complete    | Manual affordance + auto-expire; `onRefresh` / `onExecute` fallback — [actions-architecture.md](https://github.com/freemansoft/Flutter-AdaptiveCards/blob/main/docs/actions-architecture.md#root-card-refresh-payload)                                         |
...
> [!WARNING]
> **Validation:** the live submit/execute gate is `validateInputs()` (`action/default_actions.dart`), which enforces `isRequired` for all inputs, `regex` for `Input.Text`, and **`min`/`max` for `Input.Number` / `Input.Date` / `Input.Time`** (out-of-range values block the action and mark the input invalid). Note the per-input `checkRequired()` methods below are a **legacy path not wired** to this gate (tracked in [audit addendum A7](https://github.com/freemansoft/Flutter-AdaptiveCards/blob/main/docs/superpowers/specs/2026-06-17-spec-compliance-audit-addendum.md)).
>
> [!NOTE]
...
| `height`                  | Block elements                         | ⚠️ Partial     | [Column-ColumnSet-Fill-Vertical-Height.md](https://github.com/freemansoft/Flutter-AdaptiveCards/blob/main/docs/Column-ColumnSet-Fill-Vertical-Height.md)           | `auto` common; `stretch` not generally implemented                                                                                                                                                                                                                                                                                          |
...
| `layouts` (`Layout.Flow`) | Container/Column/TableCell/root (v1.6) | ⚠️ Partial     | [finish-flow design](https://github.com/freemansoft/Flutter-AdaptiveCards/blob/main/docs/superpowers/specs/2026-06-27-finish-layout-flow-design.md)                | `Layout.Flow` wrapping on `Container`, card root body, **`Column`, and `TableCell`** (spacing, item alignment, `itemWidth`, `min`/`maxItemWidth`); `itemFit: "Fit"` honored, **`itemFit: "Fill"` not yet supported**. `Layout.Stack` default. **`ColumnSet` has no `layouts` property in the spec.** `Layout.AreaGrid`/`grid.area` deferred |
...
| **Responsive layout**           | `targetWidth` ✅; `Layout.Flow` ✅ (Container, root, Column, TableCell); `itemFit: Fill`, `Layout.AreaGrid` / `grid.area` deferred | Medium — `AreaGrid` + Flow `itemFit: Fill` remain (`ColumnSet` not in spec) |
