...
| **`Container.roundedCorners`**  | _([Teams extension](https://learn.microsoft.com/en-us/microsoftteams/platform/task-modules-and-cards/cards/cards-format))_ Radius resolved via HostConfig `cornerRadius` (default 8); not yet exposed on `ColumnSet`/`Column`/`Table`/`Image` | Low — opt-in JSON flag; `ColumnSet`/`Column`/`Table`/`Image` wiring tracked separately |
