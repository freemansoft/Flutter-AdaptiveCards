...
| `layouts` (`Layout.Flow`) | Container/root (v1.6) | ⚠️ Partial     | [responsive design](https://github.com/freemansoft/Flutter-AdaptiveCards/blob/main/docs/superpowers/specs/2026-06-18-responsive-layout-targetwidth-flow-design.md) | `Layout.Flow` wrapping on `Container` + card root body (spacing, item alignment, `min`/`maxItemWidth`); `Layout.Stack` default. `itemFit` and `ColumnSet`/`Column`/`TableCell` not yet supported                                                         |
