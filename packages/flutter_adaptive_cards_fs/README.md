...
| --------------- | -------------------------------------------------------------- | ------------------------------------------------------------------------------------------ | -------------- | ------ | ---------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Input.Text      | [spec](https://adaptivecards.io/explorer/Input.Text.html)      | [learn](https://learn.microsoft.com/en-us/adaptive-cards/schema-explorer/input-text)       | ✅ Complete    | ✅ Yes | [form-inputs.md](https://github.com/freemansoft/Flutter-AdaptiveCards/blob/main/docs/form-inputs.md) | Flutter Form-based; `label` supported (v1.3+); `style: "password"` masks characters (client-side; value submitted clear-text); optional eye-icon reveal toggle controlled by HostConfig `inputs.text.revealPasswordEnabled` or per-element overlay (`revealPasswordEnabled`), default enabled; overlay preserved on `Action.ResetInputs`; `inlineAction` (v1.2+) renders the action beside the field via `cardTypeRegistry.getAction`, wired to the full submit/execute pipeline; **inline action fires on Ctrl/Cmd+Enter** when the field has focus; give the action an `iconUrl` (image URL/data URI, or the Fluent reference `icon:<name>` e.g. `icon:Send`) to show an icon. The button currently shows the icon **and** the `title` — icon-only rendering (icon _instead of_ the title, per the `inlineAction` spec) is a follow-up |
...
| Carousel          | `Carousel`       | ✅ Complete    | ✅ Yes     | -                                                                                                                             | Hub extension; child pages use `CarouselPage`. Supports `pages`, `initialPage`, `timer` (auto-advance), `orientation` (vertical), `loop`, `height` (`auto`/`stretch`) and `heightInPixels` (fixed px); default `auto` sizes to the tallest page |
| CarouselPage      | `CarouselPage`   | ✅ Complete    | ✅ Yes     | -                                                                                                                             | Child element of `Carousel`; renders `items` and `showBorder`; supports `selectAction`                                                                                                                                                          |
