...
| **`roundedCorners`**            | _([Teams extension](https://learn.microsoft.com/en-us/microsoftteams/platform/task-modules-and-cards/cards/cards-format))_ Radius resolved via HostConfig `cornerRadius` (default 8); wired on `Container`, `ColumnSet`, `Column`; not yet exposed on `Table`/`Image` | Low — opt-in JSON flag; `Table`/`Image` wiring tracked separately            |
